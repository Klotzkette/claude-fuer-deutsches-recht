...
export async function loadWorkbookRuntime(extraPackages = []) {
  ...
  const packages = ['@oai/artifact-tool', ...extraPackages];
  let runtime;
  ...
  if (!runtime) {
    throw new Error('Tabellenwerkzeuge fehlen: ' + packages.join(', ') + ' im lokalen node_modules bereitstellen oder AKTEN_NODE_MODULES auf ein vorhandenes node_modules-Verzeichnis setzen.');
  }
  ...
  return { Workbook, SpreadsheetFile, requireRuntime: runtime };
}
