...
description: >
  ...
  vollständige Cold-Start-Interview neu zu starten. Risikoeinstellung, Eskalationskontakte,
  ...
  „Playbook anpassen" oder „customize".
...
Der Nutzer hat `/ki-governance:customize` eingegeben. Er möchte etwas in seinem Praxisprofil
ändern – eine Risikoeinstellung, einen Eskalationskontakt, eine Playbook-Position, eine
Jurisdiktion, ein Ausgabeformat – ohne das gesamte Cold-Start-Interview neu zu starten und
...
- Konfiguration aus `~/.claude/plugins/config/claude-fuer-deutsches-recht/ki-governance/CLAUDE.md`
  und `company-profile.md` (eine Ebene höher)
- Beschreibung der gewünschten Änderung vom Nutzer

## Ablauf

1. **Konfiguration lesen.** CLAUDE.md und company-profile.md lesen. Falls CLAUDE.md nicht
   existiert oder noch `[PLATZHALTER]`-Werte enthält:

   > Sie haben noch kein Setup durchgeführt. Führen Sie zuerst `/ki-governance:cold-start-interview`
   > aus – customize dient der Anpassung eines bereits vorhandenen Profils.
...
2. **Anpassbare Karte anzeigen.** Auflisten, was im Profil steht, gruppiert, mit
   ...
   - **Unternehmen / Wer Sie sind** – Name, Branche, Jurisdiktionen, Phase, Praxiskontext
     *(geteilt über alle Plugins – Änderungen fließen durch `company-profile.md`)*
   ...
   - **Folgenabschätzungs-Hausformat** – FRIA-/DSFA-Abschnittsreihenfolge, Risiko-Scoring-Format,
     Stakeholder-Framing
   - **Workflow** – Aufnahme-Pfad, Ausgabeformat, Mandats-Workspace-Pfade, Prüfkadenz für
     den Policy-Monitor
...
4. **Änderung vornehmen.** Aktuellen Wert zeigen, neuen Wert abfragen, nachgelagerte
   ...
   - *Eskalationskontakt hinzufügen:* „Jeder Skill, der Eskalationen weiterleitet
     (`/use-case-triage`, `/vendor-ai-review`, `/reg-gap-analysis`), wird diesen Kontakt
     nun auf den relevanten Risikostufen einschließen."
   - *Neuer Use-Case-Register-Eintrag:* „`/use-case-triage` gleicht beim nächsten Lauf
...
5. **Bei Änderungen am gemeinsamen Profil** (Unternehmensname, Branche, Jurisdiktionen,
   Praxiskontext):
   `~/.claude/plugins/config/claude-fuer-deutsches-recht/company-profile.md` schreiben
   und vermerken:
...
6. **Abschluss.**
   ...
   > `/ki-governance:customize` jederzeit ausführen.
...
- **Abschnitt nicht löschen.** Falls der Nutzer etwas „entfernen" möchte, auf
  ...
  („Das Entfernen Ihrer Eskalationskette bedeutet, dass `/use-case-triage` eskalationswürdige
