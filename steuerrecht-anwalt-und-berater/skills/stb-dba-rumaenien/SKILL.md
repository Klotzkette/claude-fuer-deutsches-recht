---
name: stb-dba-rumaenien
description: "DBA Deutschland Rumaenien 2001. Anwendungsfall Automotive-Produktion IT Pflege Holding Beteiligungen. EU-MTRL ergaenzend. Methodenartikel Anrechnung. Output Mandanten-Memo Berechnungsbeispiel BZSt-Hinweise."
---

# DBA Deutschland-Rumaenien (2001)

## Kernsachverhalt

Das DBA-Rumaenien vom 04.07.2001 (BGBl. 2003 II S. 1594) regelt Beziehungen zum EU-Mitglied Rumaenien (Beitritt 01.01.2007). Praxisrelevant sind Automotive-Produktion (Mercedes, Continental), IT-Branche, Pflege-Arbeitnehmer in Deutschland (24-Stunden-Pflege), Holding-Strukturen.

## Kaltstart-Rueckfragen

1. Konstellation: Operativ, Pflege, IT, Holding?
2. Bei Pflege-Arbeitnehmer: Wohnsitz, Sozialversicherung A1?
3. Beteiligungshoehe?
4. Substanz?
5. Bauausfuehrung?
6. MLI-Anpassungen?
7. EU-MTRL anwendbar?
8. Ansaessigkeitsbescheinigung?

## Rechtlicher Rahmen

### Primaernormen
- **DBA Deutschland-Rumaenien vom 04.07.2001**, in Kraft 17.12.2003, anwendbar ab 01.01.2004 (BGBl. 2003 II S. 1594).
- **OECD-MA**.
- **§§ 34c, 32b, 50d EStG**, § 20 AStG.
- **MTRL** (§ 43b EStG), **ZinsLizenzRL** (§ 50g EStG).
- **MLI**: Deutschland und Rumaenien sind MLI-Unterzeichner; das DBA-Rumaenien ist Stand 01.01.2026 nicht als Covered Tax Agreement im deutschen BEPS-MLI-Anwendungsgesetz gelistet; aktuellen Stand beim BMF pruefen.

### Leitentscheidungen und BMF-Schreiben
- BMF-Schreiben zur steuerlichen Behandlung des Arbeitslohns nach DBA (zuletzt 12.12.2023, BStBl. I 2023, 2193) — einschliesslich Entsendungssituationen rumaenischer Pflege-Arbeitnehmer nach Art. 15 DBA-Rumaenien.

## Land-spezifisches

### Ansaessigkeit (Art. 4)
- OECD-Tie-Breaker.

### Betriebsstaette (Art. 5)
- Bauausfuehrung **12 Monate**.

### Aktive Einkuenfte
- Loehne Art. 15: 183-Tage-Regel.
- **Pflege-Arbeitnehmer**: Wohnsitz Rumaenien, Arbeitsort Deutschland — wenn 183-Tage in DE: deutsche Steuerpflicht.

### Passive Einkuenfte
- **Dividenden** (Art. 10): 5 Prozent bei direkter Beteiligung von mindestens 10 Prozent am Kapital; 15 Prozent in allen anderen Faellen. EU-MTRL Schachtel 0 Prozent.
- **Zinsen** (Art. 11): 3 Prozent des Bruttobetrags im Quellenstaat (Art. 11 Abs. 2 DBA-Rumaenien); solange Deutschland nach innerstaatlichem Recht keine Abzugsteuer auf Zinsen erhebt, gilt faktisch 0 Prozent von deutscher Seite (Art. 11 Abs. 4 DBA-Rumaenien).
- **Lizenzgebuehren** (Art. 12): 3 Prozent des Bruttobetrags im Quellenstaat. EU-ZinsLizenzRL (§ 50g EStG) bei verbundenen Unternehmen 0 Prozent.

### Vermeidungs-Methode (Art. 23)
- Freistellung mit Progressionsvorbehalt aktive Einkuenfte.
- Anrechnung passive Einkuenfte.

### Besonderheiten
- **Pflege-Arbeitnehmer**: A1-Bescheinigung Sozialversicherung; Lohnsteuer ueber 183-Tage-Regel.
- **Niedrige rumaenische KSt 16 Prozent** — CFC-Pruefung.
- **Pensionen** Art. 18.

## Workflow

### Phase 1 — DBA-Anwendbarkeit
### Phase 2 — Ansaessigkeit klaeren
### Phase 3 — Einkunftsart einordnen
### Phase 4 — Verteilungs- und Methodenartikel
### Phase 5 — Erstattung / Erklaerung

## Output

- Mandanten-Memo.
- Berechnungsbeispiel.
- BZSt-Antrag.

## Strategie und Praxis-Tipps

- Pflege-Arbeitnehmer Rumaenien: A1-Bescheinigung Sozialversicherung; bei ueber 183 Tagen DE: Lohnsteuer.
- Niedrige KSt Rumaenien: CFC, Pillar Two.
- Automotive-Werke in Rumaenien: Betriebsstaette pruefen, Werksvertraege.
- EU-MTRL vorrangig fuer Schachtel.
- BZSt-Erstattung bei Lizenzen.

## Praktiker-Tipps der alten Hasen

- **Erstattungsverfahren RO → DE-Mandant**: Antrag bei ANAF (Agenția Națională de Administrare Fiscală); Online-Portal "Spațiul Privat Virtual" der ANAF (vom Anwender mit aktuellem Stand des rumaenischen Online-Portals zu verifizieren). Ansaessigkeitsbescheinigung "Certificat de rezidență fiscală" der ANAF als Anlage.
- **Sprache des DBA**: Authentische Textfassungen Deutsch und Rumaenisch; bei strittiger Auslegung beide vergleichen.
- **Lokaler Berater**: Bei rumaenischer SRL (GmbH-Aequivalent) mit deutschem Anteilseigner empfiehlt sich Hinzuziehung eines rumaenischen "consultant fiscal" wegen RO-Pruefungspraxis (sehr formalistisch, viele lokale Steuerarten: Sondersteuern Banken, OMV-Petroleum, Telekom).
- **Apostille**: nicht erforderlich zwischen EU-Mitgliedstaaten (RO seit 2007).

## Edge Cases und Was-Pruefer-Triggert

- **Mikrobesteuerung RO**: bis Ende 2022 hatte RO ein attraktives Mikrounternehmen-Regime (1 bzw. 3 Prozent auf Umsatz statt 16 Prozent KSt); ab 2023 stark eingeschraenkt (Umsatzgrenze 500.000 EUR, dann 250.000 EUR, ggf. weiter reduziert — vom Anwender mit aktuellem RO-Steuerrecht zu verifizieren). Hat AStG-Niedrigsteuer-Relevanz.
- **Pflege-Arbeitnehmer Rumaenien**: Klassische 24-Stunden-Pflege in DE — Vertragsgestaltung mit RO-Service-Gesellschaft, EU-Entsendung A1; 183-Tage-Regel sorgfaeltig pruefen, da Familienbezuege nach RO oft Lebensmittelpunkt nahe legen.
- **Automotive-Standorte RO**: Dacia (Renault), Daimler, Continental, Ford in Craiova — bei Werksvertraegen 12-Monatsfrist akribisch dokumentieren.
- **§ 50d Abs. 9 EStG Subject-to-Tax**: bei RO-Quellen wegen ggf. niedriger lokaler Belastung pruefen.
- **MLI nicht Covered Tax Agreement**: PPT nicht ueber MLI; nationales § 50d Abs. 3 EStG und § 42 AO maßgeblich.

## Trade-offs

| Trade-off | Pfad A | Pfad B | Empfehlung |
|---|---|---|---|
| Pflege-Werkvertrag RO-Gesellschaft vs. AUe | RO-Werkvertragsgesellschaft mit Entsendung: keine deutsche Lohnsteuer bei korrektem Werkvertrag; A1 | direkte AUe nach AUeG | BAG/Pruefer hinterfragt oft Werkvertragsgestaltung; bei Pflege-24h-Konstellation hohe Risiken — sorgfaeltige Vertragsgestaltung und Substanz |
| EU-MTRL vs. DBA-Schachtel | EU-MTRL: 0 Prozent ab 10 Prozent | DBA-RO: 5 Prozent ab 10 Prozent | EU-MTRL vorrangig |
| RO-KSt 16 Prozent (Stand 05/2026 vom Anwender zu verifizieren) | knapp ueber AStG-Schwelle 15 Prozent — keine CFC-Probleme | bei Mikrosteuer-Regime: deutlich unter 15 Prozent — CFC bei passiv | bei Mikrosteuer immer CFC pruefen; bei Standard-KSt 16 Prozent regelmaessig kein CFC |

## Berechnungsbeispiel

Pflegekraft mit Wohnsitz und Familie in Rumaenien, Arbeitseinsatz in DE bei privatem Auftraggeber, 200 Tage in DE im Kalenderjahr. Bruttolohn ueber RO-Werkvertragsgesellschaft 18.000 EUR:

- Ueberschreitung 183-Tage-Schwelle: DE-Lohnsteuerpflicht des Arbeitnehmers (Art. 15 DBA-RO).
- A1-Bescheinigung Sozialversicherung: bei korrekter Entsendung RO-SV-Pflicht; bei zu langer Aufenthaltsdauer EU-VO 883/2004 wechselt zu DE.
- DE-Lohnsteuer: bei 18.000 EUR Steuerklasse 1 (Singles) rund 1.620 EUR (vereinfacht, ohne Familienkomponenten).
- Erklaerungspflicht in RO: Welteinkommen, mit Anrechnung der DE-Steuer (RO ist Methodenartikel nach DBA-RO Anrechnung).
- Praxisproblem: oft fehlende A1, mangelnde Lohnabrechnung — Pruefer triggert auf undokumentierte Konstellationen.

## Querverweise

- `stb-dba-grundprinzip-oecd-musterabkommen`, `stb-dba-ansaessigkeit-tie-breaker-rules`, `stb-dba-methodenartikel-anrechnung-vs-freistellung`, `stb-dba-quellensteuer-erstattung-bzst-50c-estg`, `stb-dba-bulgarien`, `stb-dba-ungarn`.

## Quellen und Updates

Stand: 05/2026. DBA-Rumaenien 04.07.2001, in Kraft 17.12.2003 (BGBl. 2003 II S. 1594). MLI-Status: nicht als Covered Tax Agreement im deutschen BEPS-MLI-Anwendungsgesetz gelistet (Stand 01.01.2026). Dividenden 5/15 Prozent, Zinsen 3 Prozent (faktisch 0% DE-Seite), Lizenzen 3 Prozent. Aktuellen Stand im BMF-DBA-Verzeichnis pruefen.

<!-- AUDIT 27.05.2026 | welle 6 | 7 Marker aufgeloest: 7 geprueft und bestaetigt (BGBl-Fundstelle korrigiert 2003 II S. 1594; MLI-Status geklaert; Quellensteuer-Saetze bestaetigt; BMF-Schreiben Pflegekraefte neu formuliert), 0 gestrichen -->
