---
name: dba-quellensteuer-atlas-weltweit
description: "Für DBA-Quellensteuer-Atlas: ordnet Norm, Beweislast und Gegenargument; Ergebnis: Schnittstellenkarte mit Zuständigkeits- und Nachweisfragen."
---

# DBA-Quellensteuer-Atlas

## Fachlicher Kern — Steuerrecht
- **Problemfokus dieses Skills:** Bleibe beim konkreten Titel `DBA-Quellensteuer-Atlas` und löse die dort angelegte Fachfrage; arbeite mit konkreten Tatbestandsmerkmalen, Beweisfragen und dem unmittelbar benötigten Arbeitsprodukt. Routingfragen bleiben Hilfsmittel, wenn Frist, Zuständigkeit oder Verfahrensart offen sind.
- **Arbeitsmodus:** Erst Steuerart, Zeitraum, Verwaltungsstand, Frist/Festsetzung, Zuständigkeit, Form/Portal und Beleglage klären; dann BMF-Verwaltungslinie von BFH-Rechtsprechung und Gesetz trennen.
- **Outputpflicht:** Steuerartenmatrix, BMF-Radar, Einspruchsbaustein, ELSTER-/Portal-To-do, Risikoampel, DBA-/GrESt-/USt-Tabelle oder Mandantenmemo.
- **Fehlerbremse:** Tragende Normen/Entscheidungen live oder aus der Akte verifizieren; Rechtsprechung nur mit Gericht, Entscheidungsform, Datum, Aktenzeichen und frei prüfbarer Quelle. Keine BeckRS-, juris-, Kommentar- oder Aufsatz-Blindzitate aus Modellwissen.

## Drei Ebenen

1. **Nationales Quellenstaatrecht:** Gibt es überhaupt Steuerabzug?
2. **DBA:** Begrenzung oder Ausschluss des Quellenstaatsrechts.
3. **Verfahren:** Freistellung vorab oder Erstattung danach; Formulare, Ansässigkeitsbescheinigung, Fristen.

## Einkunftsarten

- Dividenden: Art. 10 DBA, § 43b EStG/EU-Mutter-Tochter-Richtlinie, § 50d Abs. 3 EStG, § 50j EStG.
- Zinsen: Art. 11 DBA, nationale Kapitalertragsteuer, EU-Zins-/Lizenzrichtlinie soweit einschlägig.
- Lizenzen: Art. 12 DBA, § 50a EStG, § 50c EStG, Software/Know-how/Service abgrenzen.
- Künstler/Sportler: Art. 17 DBA, § 50a EStG, Durchgriff auf Künstlergesellschaft.
- Technische Dienstleistungen: nur wenn konkretes DBA oder Quellenstaatrecht eine Sonderlogik enthält.

## Praktiker-Tipps "Schnell zum Bescheid / BZSt-Erstattungs-Tipps"

- **Quellenstaat zuerst, Empfaengerstaat danach**: erst nationales Quellensteuerrecht des Auszahlungsstaats, dann DBA, dann Empfaengerstaats-Behandlung (Anrechnung/Freistellung). Reihenfolge im Memo strikt einhalten.
- **BZSt-Online-Portal BOP** als Standardweg: Antraege auf Freistellung (§ 50c Abs. 2 EStG) und Erstattung (§ 50c Abs. 3 EStG) digital über bzst.de/BOP — Formularkatalog laufend aktualisiert. Vom Anwender mit aktuellem BOP-Stand abzugleichen.
- **Ansaessigkeitsbescheinigung Heimatstaat**: IRS Form 6166 (USA, Antrag über Form 8802 — Bearbeitung 6-8 Wochen), HMRC Certificate (UK), CFR-1 (Polen), CRA (Kanada), spanisch/italienisch/franzoesisches Äquivalent — Bearbeitung 4-8 Wochen. Mehrjaehrig gueltige Bescheinigungen anfragen, wenn möglich.
- **DAV-Verfahren** für institutionelle Anleger (Banken/Wertpapierdienstleister): Massen-Erstattungs-Verfahren statt Einzelantraege — deutlich schneller.
- **Apostille/Konsularbeglaubigung** bei Drittstaaten (USA, VAE, Australien) — Vorlaufzeit 6-10 Wochen.
- **Originalbelege vs. Kopien**: BZSt akzeptiert beglaubigte Kopien der Ansaessigkeitsbescheinigung; Steuerbescheinigung des deutschen Schuldners im Original.
- **Antragsfenster Q1**: Erstattungs-Antraege im Januar bis Maerz kuerzere Bearbeitungszeit als Q4.
- **Eilantrag funktioniert nicht**: BZSt verfolgt seriellen Bearbeitungspfad. Stattdessen: Freistellung vorab.

## Trade-off-Tabelle

| Trade-off | Pfad A | Pfad B | Empfehlung |
|---|---|---|---|
| Freistellungsbescheinigung vorab vs. Erstattung nachher | vor Zahlung — schneller Cashflow, 3-Jahres-Geltung | nach Einbehalt rueckwirkend — 4-Jahres-Frist | bei regelmäßigen Zahlungen Pfad A |
| MTRL/ZinsLizenzRL vs. DBA-Hoechstsatz | EU-RL 0 Prozent bei Verbundenheit | DBA-Hoechstsatz | EU-RL vorrangig, wenn Voraussetzungen erfuellt |
| Erstattung jetzt vs. später | sofort Antrag, Bearbeitung 12-24 Monate | später Antrag, gleiche Bearbeitung — aber Frist droht | bei hohen Betraegen frueh stellen |
| Substanz aufbauen vs. § 50d Abs. 3 EStG-Risiko hinnehmen | Substanz (Personal, Buero, Geschäftstaetigkeit) | "Briefkasten" oder durchgereichte Holding | bei wesentlichen Erstattungs-Volumina Substanz aufbauen |
| Bei Drittstaat ohne DBA: Quellensteuer akzeptieren oder strukturieren | hinnehmen + Anrechnung § 34c EStG | EU-Tochter als Verbindungsstueck | bei reiner Steuergestaltung scheitert Pfad B oft an § 50d Abs. 3 EStG |
| BZSt-Erstattung vs. MAP | unilaterale Erstattung beim BZSt | Verstaendigungsverfahren bei Doppelbesteuerung trotz DBA | bei klarem DBA-Anspruch Pfad A; bei strittiger Qualifikation Pfad B |

## Was Reviewer/Prüfer triggert

- **Drei-Ebenen-Prüfung verkuerzt**: Memo springt direkt zum DBA-Hoechstsatz, ohne nationales Quellenstaatrecht und Verfahren zu prüfen.
- **§ 50d Abs. 3 EStG-Substanz nicht dokumentiert** bei Konzern-Holdings.
- **§ 50j EStG-Cum-Cum-Frist nicht gewahrt** bei Dividenden — 45-Tage in 91-Tage-Fenster.
- **Ansaessigkeitsbescheinigung fehlt** oder veraltet (gilt regelmaessig nur für ein Jahr).
- **Falsches BZSt-Formular** oder Bezeichnung — Antrag wird zurueckgewiesen.
- **MTRL- vs. DBA-Wahl nicht begruendet** — Prüfer fragt: warum nicht die guenstigere Variante?
- **Software-Lizenz vs. Software-Verkauf** falsch qualifiziert — Art. 12 vs. Art. 7.

## Dokumenten-Checkliste pro Antrag

| Dokument | Lizenz | Dividende | Zinsen | Kuenstler/Sportler |
|---|---|---|---|---|
| Ansaessigkeitsbescheinigung Heimat-FA | ja | ja | ja | ja |
| Steuerbescheinigung deutscher Schuldner (KapErtSt) | ja | ja | ja | ja |
| Lizenzvertrag | ja | nein | nein | nein |
| Konzern-/Beteiligungsstruktur | nur bei verb. Unternehmen | ja | ggf. | nein |
| Verguetungsabrechnung | ja | nein | nein | ja |
| Substanzdokumentation (§ 50d Abs. 3 EStG) | bei Holding | bei Holding | bei Holding | bei Star-Company |
| Cum-Cum-Frist-Dokumentation (§ 50j EStG) | nein | ja | nein | nein |
| Werbungskosten-Nachweise (Nettoabzug) | nein | nein | nein | ja (bei § 50a Nettoabzug) |
| Vertrag Star-Company / Auftrittsorte | nein | nein | nein | ja |
| Apostille / Konsularbeglaubigung Drittstaat | bei Bedarf | bei Bedarf | bei Bedarf | bei Bedarf |

## Quellensteuer-Rechenblatt (Schema)

| Schritt | Prüfung | Ergebnis |
|---|---|---|
| 1. Quellenstaat-Nationalrecht | Gibt es Steuerabzug? Welcher Satz? | [Satz] |
| 2. EU-RL (MTRL/ZinsLizenzRL) | Voraussetzungen erfuellt? | [0 Prozent / nicht anwendbar] |
| 3. DBA-Hoechstsatz | Aus DBA-Text | [Satz] |
| 4. Anti-Treaty-Shopping § 50d Abs. 3 EStG | Substanz nachgewiesen? | [bestanden / scheitert] |
| 5. Cum-Cum § 50j EStG | 45/91-Tage erfuellt? | [ja / nein] |
| 6. Beneficial Ownership / PPT | wirtschaftlicher Eigentümer / Hauptzweck? | [ja / nein] |
| 7. Effektiver Quellensteuersatz | Minimum aus 1 / EU-RL / DBA | [Satz] |
| 8. Verfahren | Freistellung vorab oder Erstattung | [Antragsweg] |
| 9. Anrechnung Empfaengerstaat | § 34c EStG / lokale Anrechnung | [Hoechstbetrag] |

## Sonderfaelle weltweit (Auswahl)

- **Brasilien**: technische Dienstleistungen oft als Lizenz qualifiziert (15 Prozent Quellensteuer); national Auslegungs-Praxis weitreichend.
- **Indien**: Equalisation Levy auf digitale Dienste (eigene Prüfung), Service-PE-Tatbestand.
- **China**: Service-PE bei 6 Monaten Anwesenheit; eigene Quellensteuer-Verfahren mit lokalen Bescheinigungen.
- **Russland**: DBA-Anwendung ab 01.01.2024 in voller Reichweite suspendiert (konsolidierter BMF-Stand 15.01.2024 und Wirkung der StAbwV-Listung) — keine BZSt-Entlastung mehr. Altvorgaenge bis 07.08.2023 mit vollem DBA-Schutz; Uebergangszeitraum 08.08.2023 bis 31.12.2023 sorgfaeltig prüfen (Details in `stb-dba-russland-suspendierung-2024`).
- **VAE**: DBA seit 01.01.2022 ausser Kraft — kein Schutz.
- **Hong Kong**: eigenes DBA (2003) — nicht CN-DBA anwenden.
- **USA**: LOB-Klausel zusaetzlich zum PPT — eigenstaendiger Anti-Missbrauchstest.
- **UK**: Brexit — keine MTRL / ZinsLizenzRL mehr.

## Anti-Missbrauchspruefung (Mustertabelle)

| Test | Norm | Befund | Status |
|---|---|---|---|
| § 50d Abs. 3 EStG Substanz | EStG | [Personal/Buero/Geschäftstaetigkeit] | [bestanden / scheitert] |
| § 50j EStG Cum-Cum | EStG | [45/91-Tage] | [erfuellt / nicht] |
| Beneficial Ownership | DBA | [Wirtschaftlicher Eigentümer?] | [ja / nein] |
| PPT (MLI) | DBA | [Hauptzweck Vorteil?] | [Vorteil zu versagen / nicht] |
| LOB (US-DBA, ggf. andere) | DBA Art. [...] | [Qualified Person?] | [ja / nein] |
| § 4j EStG Lizenzschranke | EStG | [schaedliche IP-Box im Quellenstaat?] | [Abzug eingeschraenkt / nicht] |
