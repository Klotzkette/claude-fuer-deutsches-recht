...
description: >
  Referenz: VERALTET — bitte `/draft` verwenden. Diese Skill wurde in die
...
Diese Skill akzeptiert keine Eingaben. Für alle Formular- und Schriftstückaufgaben: `/draft [Schriftstücktyp]`.
...
### Relevante Normen für die Nachfolge-Skill `/draft`
...
**Stattdessen `/draft [Schriftstücktyp]` verwenden.**

```
/draft beratungshilfe-antrag
/draft pkh-antrag
/draft widerspruch-nebenkostenabrechnung
/draft mahnschreiben
```

Vollständiger Ablauf in `skills/draft/SKILL.md`:
...
Keine Ausgabe — diese Skill ist inaktiv. Weiterleitung auf `/draft`.

## Beispiel

Statt `/form-generation beratungshilfeantrag`:

```
/draft beratungshilfe-antrag
```

Der Befehl `/draft` füllt das Formular BerH 1 aus den Fallnotizen, kennzeichnet fehlende Pflichtangaben mit `[TATSACHE FEHLT: ...]` (z. B. Kontonummer für Bedürftigkeitsnachweis), und leitet nach Fertigstellung in die Supervisoren-Prüfung.

## Risiken und typische Fehler

- **Verweis auf diese Skill in älteren Unterlagen:** Bestehende Handreichungen, Semesterskripte oder Tutorenmaterialien, die auf `/form-generation` verweisen, auf `/draft` umschreiben.
...
Nicht anwendbar (Weiterleitungs-Skill). Für alle Quellenangaben bei konkreten Schriftstücken und Formularen: `skills/draft/SKILL.md`, Sektion „Quellenpflicht".
