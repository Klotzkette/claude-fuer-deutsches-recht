---
name: streitstellen-katalog-zwanzig-typische
description: "Für Streitstellen — die 20 wichtigsten Kontroversen: ordnet Norm, Beweislast und Gegenargument; Ergebnis: Prüfprodukt mit Risiko und nächstem Schritt."
---

# Streitstellen — die 20 wichtigsten Kontroversen

## Wann diesen Skill nutzen

In Klausur oder Schriftsatz die Streitstellen-Erkenntnis zeigen — nicht jede aufrollen, aber die einschlaegige sicher behandeln. Dieser Skill liefert pro Streitstelle drei Dinge: Pro-Argument, Contra-Argument, herrschende Meinung mit Folgerung.

## 1. Wesensgehaltsgarantie — absolut oder relativ?

| | Argument |
|---|---|
| **Absolut** | Art 19 II GG zieht eine unueberschreitbare Grenze; Wuerde-Bezug der Grundrechte |
| **Relativ (Alexy)** | Wesensgehalt ist Ergebnis der Verhältnismäßigkeitspruefung; was die Prüfung haelt, wahrt auch den Wesensgehalt |
| **Folgerung** | BVerfG seit Luftsicherheitsgesetz (BVerfGE 115, 118) wieder absolut; klausurmaessig absolut prüfen |

## 2. Drei-Stufen-Lehre Art 12 GG — noch zeitgemaess?

| | Argument |
|---|---|
| **Pro** | Strukturanker; differenziert nach Eingriffstiefe |
| **Contra** | BVerfG schaerft Stufenuebergaenge weich; sukzessive Aufloesung in die allgemeine Vier-Stufen-Prüfung |
| **Folgerung** | In der Klausur weiter anwenden, aber Stufenuebergaenge nicht ueberbewerten |

## 3. Eingriffsbegriff — klassisch oder modern?

| | Argument |
|---|---|
| **Klassisch** | Final, rechtsfoermig, unmittelbar, mit Befehl und Zwang |
| **Modern** | Jede Beeintraechtigung der grundrechtlich geschuetzten Sphaere; auch mittelbar-faktisch (Subventions-Entzug, oeffentliche Warnung) |
| **Folgerung** | Moderner Eingriffsbegriff ist h. M. seit Glykol-/Osho-Linie (BVerfGE 105, 252) |

## 4. Geeignetheit — objektiv ex ante oder objektiv ex post?

| | Argument |
|---|---|
| **Ex ante** | Beurteilung nach Erkenntnislage zum Erlasszeitpunkt; schuetzt Gesetzgeber-Prognose |
| **Ex post** | Korrektur bei später widerlegter Eignung; Pflicht zur Beobachtung und Nachbesserung |
| **Folgerung** | BVerfG kombiniert: ex ante bei Prognose, ex post Beobachtungs- und Nachbesserungspflicht (Beobachtungspflicht) |

## 5. Erforderlichkeit — Kostengleichheit beim milderen Mittel?

| | Argument |
|---|---|
| **Strikt** | Milderes Mittel auch bei hoeheren Kosten |
| **Relativ** | Wirtschaftliche Ueberlegungen begrenzen Pflicht zur Alternativ-Wahl |
| **Folgerung** | Grundrechtsvorrang ueberwiegt; Kostenargument nur in Ausnahmefaellen tragfaehig |

## 6. Angemessenheit — Bauchgefuehl oder formalisierte Abwaegung?

| | Argument |
|---|---|
| **Strukturierte Abwaegung (Alexy)** | Triadische Skala, Gewichtsformel |
| **Wertungsgesteuerte Abwaegung** | Wertungen sind nicht formalisierbar |
| **Folgerung** | Strukturierter Aufbau steigert Nachvollziehbarkeit, ersetzt aber keine Wertung |

## 7. Schutzbereichsabgrenzung — eng oder weit?

| | Argument |
|---|---|
| **Weit** | Schutzbereich grosszuegig oeffnen, Verfeinerung auf Schranken-Ebene |
| **Eng** | Schutzbereich praezise abgrenzen, weniger Schranken-Prüfung |
| **Folgerung** | h. M. weit; Filterung auf Schranken-Ebene |

## 8. Verfassungsimmanente Schranke — was zaehlt?

| | Argument |
|---|---|
| **Nur Grundrechte Dritter** | Schmal, Privilegierung der Grundrechtstraeger |
| **Verfassungsguete inkl. Staatszielen** | Breit, auch Lebensschutz, Umwelt, Klima, Staatsfinanzen |
| **Folgerung** | BVerfG breit; siehe Klimaschutz-Beschluss zur Aufnahme der Klimagrenze |

## 9. Wechselwirkungslehre — Art 5 GG-Spezial?

| | Argument |
|---|---|
| **Spezial** | Nur für Meinungsfreiheit, BVerfGE 7, 198 |
| **Allgemein** | Strukturanaloge Anwendung bei anderen vorbehaltlich/qualifiziert geschuetzten Grundrechten |
| **Folgerung** | Methodisch allgemein (Lueth-Linie); siehe `meinungsfreiheit-wechselwirkungslehre-wunsiedel` |

## 10. Schutzpflicht und Untermassverbot

| | Argument |
|---|---|
| **Symmetrisch zum Uebermassverbot** | Vier-Stufen-Prüfung auch gegen Untaetigkeit |
| **Asymmetrisch** | Untermass nur evidente Unwirksamkeit |
| **Folgerung** | Klimabeschluss verschaerft Untermass; in dynamischen Bereichen nach Symmetrie prüfen |

## 11. Einschaetzungsspraerogative — wie weit reicht sie?

| | Argument |
|---|---|
| **Weit** | Bei wissenschaftlicher Unsicherheit, BVerfG-Bundesnotbremse |
| **Eng** | Bei intensiven Eingriffen Kontrolldichte verschaerft |
| **Folgerung** | Kontrolldichte korreliert mit Eingriffsintensitaet — `einschaetzungspraerogative-kontrolldichte` |

## 12. Praktische Konkordanz vs. Abwaegung — Verhältnis?

| | Argument |
|---|---|
| **Methodische Vorbedingung** | Praktische Konkordanz prüfen, bevor Vier-Stufen greifen |
| **Bestandteil der Stufe 4** | Konkordanz ist nur ein Aspekt der Angemessenheit |
| **Folgerung** | Beide Sichten zulässig; klausurmaessig sauber trennen |

## 13. Grundrechtsverzicht — zulässig?

| | Argument |
|---|---|
| **Zulässig bei Disponibilitaet** | Eigentum, Berufsfreiheit, Versammlungsfreiheit (bei freiwilligem Verzicht) |
| **Unzulaessig bei Wuerde-/Kernbereich** | Wuerde, Kernbereich der Privatsphaere |
| **Folgerung** | Differenzieren nach Schutzgut |

## 14. Wesentlichkeitstheorie — wie konkret?

| | Argument |
|---|---|
| **Streng** | Jede grundrechtsrelevante Frage muss der Gesetzgeber selbst regeln |
| **Pragmatisch** | Verordnungsermaechtigungen ausreichend, wenn Rahmen bestimmt |
| **Folgerung** | BVerfG verschaerft seit Numerus-Clausus-Linie; klausurmaessig prüfen, ob Parlamentsvorbehalt einschlaegig |

## 15. Verhältnismäßigkeit von Strafen

| | Argument |
|---|---|
| **Vollstaendig anwendbar** | Vier-Stufen auf Strafhoehe |
| **Modifiziert** | Schuldgrundsatz hat Vorrang; Verhältnismäßigkeit als Korrektiv |
| **Folgerung** | Schuld-/Tatangemessenheit als Stufen-2-3-Bezug; Stufe 4 als Schuldhoehenkontrolle |

## 16. Drittwirkung im Privatrecht

| | Argument |
|---|---|
| **Unmittelbar** | Grundrechte gelten direkt zwischen Privaten (selten) |
| **Mittelbar** | Grundrechte als Werteordnung wirken durch Generalklauseln (h. M., Lueth) |
| **Folgerung** | Mittelbar, mit Ausnahmen z. B. AGG, BetrVG |

## 17. Klimaschutz und intertemporale Freiheit

| | Argument |
|---|---|
| **Generationengerechtigkeit als Stufe-4-Faktor** | Eingriff in Freiheit zukuenftiger Generationen begruendet schaerfere Kontrolldichte |
| **Verfassungsrechtlich ueberzogen** | Subjektive Rechte zukuenftiger Generationen sind nicht aktivierbar |
| **Folgerung** | BVerfGE 157, 30 setzt erstes durch — siehe `klimaschutz-bverfge-157-30` |

## 18. Verfahrensgerechtigkeit als Stufe-3-Bestandteil

| | Argument |
|---|---|
| **Verfahrenssicherungen sind Erforderlichkeitsfrage** | Eingriff mit Sicherungen ist milder; Online-Durchsuchungs-Linie |
| **Separater Prüfpunkt** | Verfahrenssicherungen sind eigene Frage des effektiven Grundrechtsschutzes |
| **Folgerung** | BVerfG verarbeitet im Rahmen Erforderlichkeit + Bestimmtheit |

## 19. EuGH/EGMR-Schutzniveau — Mindeststandard?

| | Argument |
|---|---|
| **Ergaenzungsschicht** | Charta/EMRK als Mindest, deutsches GG kann mehr |
| **Solange-Logik** | Identitaetsvorbehalt, BVerfGE 73, 339 (Solange II) |
| **Folgerung** | Solange-II-Linie + Honeywell + BVerfGE 152, 152 (Recht auf Vergessen II); im Anwendungsbereich Unionsrecht: GRCh als unmittelbarer Maßstab |

## 20. Verhältnismäßigkeit als globales Strukturprinzip

| | Argument |
|---|---|
| **Ja (Barak)** | Vierstufige Prüfung in Kanada, Suedafrika, EuGH, EGMR, Indien, Israel |
| **Nein** | Im U.S.-Recht abweichende Tiers-of-Scrutiny-Logik |
| **Folgerung** | Konvergenz mit Ausnahmen; Argument für Vergleichbarkeit, nicht für Universaluebernahme |

## Anschluss

- Vertiefung der Vier-Stufen: `vierstufige-schranken-schranke`
- Theorie Alexy: `theorie-alexy-prinzipientheorie`
- Rechtsvergleich: `usa-tiers-of-scrutiny`, `kanada-oakes-test-uebersicht`, `suedafrika-section-36-uebersicht`
- Klausurschema: `klausur-pruefungsschema-kompakt`
