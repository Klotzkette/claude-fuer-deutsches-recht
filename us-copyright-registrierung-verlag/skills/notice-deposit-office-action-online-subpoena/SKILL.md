---
name: notice-deposit-office-action-online-subpoena
description: "Notice Deposit Office Action Online Subpoena: bündelt 4 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert."
---

# Notice Deposit Office Action Online Subpoena

## Arbeitsbereich

Dieser Skill bündelt 4 sachlich verwandte Arbeitsschritte rund um **Notice Deposit Office Action Online Subpoena** im Plugin Us Copyright Registrierung Verlag. Die Prüffelder bauen aufeinander auf: zuerst das tragende Feld nach der konkreten Aktenlage bestimmen, dann ergänzende Felder nur dort heranziehen, wo dieselbe Akte mehrere Punkte trägt. Rolle, Frist, Zuständigkeit, Beweislast und gewünschter Output bleiben dabei sauber getrennt.

## Prüffelder

| Prüffeld | Fokus |
| --- | --- |
| `notice-deposit-registration-ch4` | Verbindet Notice, Deposit, Registration, Fehlerfolgen, Best Edition und Litigation Readiness in Chapter 4. |
| `office-action-rueckfragen-45-tage` | Office Actions und Rückfragen: Monitoring, Antwortfristen, fehlende Angaben, Claim-Begrenzung, Nachlieferungen und Eskalation zu US-Counsel. |
| `online-subpoena-512h` | Prueft Subpoena zur Identifizierung mutmaßlicher Verletzer, Notice-Voraussetzungen, Provider-Rolle und Datenschutzfolgen. |
| `ownership-transfer-writing` | Prueft Übertragung, exklusive Lizenz, nonexclusive license, writing requirement und chain of title. |

## Arbeitsweg

Im Plugin Us Copyright Registrierung Verlag gilt für **Notice Deposit Office Action Online Subpoena**: zuerst das tragende Prüffeld auswählen, dessen Tatsachen im konkreten Fall wirklich angelegt sind. Ergänzende Prüffelder nur heranziehen, wenn dieselbe Akte sie trägt. Rollen, Fristen, Zuständigkeit, Anspruchs- oder Verfahrensgrundlage, Beweislast und gewünschten Output sauber getrennt halten. Tragende Normen und Fundstellen nach `references/quellenhygiene.md` verifizieren.


## Prüffelder im Detail

## 1. `notice-deposit-registration-ch4`

**Fokus:** Verbindet Notice, Deposit, Registration, Fehlerfolgen, Best Edition und Litigation Readiness in Chapter 4.

# Notice, Deposit und Registration Chapter 4

## Fachkern: Notice, Deposit und Registration Chapter 4
- **Spezialgegenstand:** Notice, Deposit und Registration Chapter 4. Die Prüfung setzt bei der konkreten Sachfrage an und endet mit einem verwertbaren Arbeitsergebnis.
- **Normen-/Quellenanker:** Title 17 U.S.C., Copyright Office Compendium, eCO-Verfahren, deposit/best edition, DMCA, fair use, termination, work made for hire und international treaties.
- **Entscheidende Weiche:** Werkart, Autor/Rechteinhaber, Veröffentlichung, Deposit, Claim/Exclusion, Registrierungsdatum, Enforcement-Ziel und US-Prozessnutzen trennen.
- **Arbeitsprodukt:** Erzeuge eine konkrete Prüf- oder Entscheidungsmatrix mit Norm, Tatbestand, Beleg, Einwand, Risikoampel und nächstem Schritt; Anschluss-Skills nur bei echter Vertiefung nennen.


## Zweck

Das US-Copyright-Act-Plugin deckt Title 17 nicht nur als Registrierungsab, sondern als Praxiswerkzeug fuer Rechteinhaber, deutsche Verlage, Plattformen, Produzenten, Lizenznehmer und Prozessvorbereitung.

Dieser Skill macht aus dem Thema **Notice, Deposit und Registration Chapter 4** eine belastbare Arbeitsroute: erst Rolle und Ziel, dann Code-Anker, Tatsachen, Dokumente, Risiken, offene US-Counsel-Fragen und verwertbarer Output. Er ist kein Ersatz für zugelassenen US-Counsel, aber er soll die deutsche Kanzlei, Rechtsabteilung oder den Verlag so vorbereiten, dass US-Counsel sofort mit einer geordneten Akte weiterarbeiten kann.

## Code- und Quellenanker

- 17 U.S.C. §§ 401-412
- 37 C.F.R. Part 202
- USCO registration portal


Aktuelle Fassungen, Forms, Fees, Local Rules und Court-Practice immer live über offizielle Quellen prüfen. Keine Modellzitate zu US-Rechtsprechung verwenden, wenn Gericht, Datum, Aktenzeichen/Docket und freie Quelle nicht geprüft sind.

## Intake-Fragen

- Ist Notice relevant oder historisch?
- Mandatory deposit oder registration deposit?
- Welche Litigation-Ziele hängen an Timing?


## Workflow

1. Notice-/deposit-Historie prüfen.
2. Registration route wählen.
3. § 411 und § 412 Risiken markieren.


## Output

- Chapter 4 route memo
- Deposit checklist
- Registration timing note


## Tiefencheck für die Akte

Arbeite hier nicht nur abstrakt, sondern als Rechte-, Werk- und Beweisfilter:

- Welcher Antragstyp passt wirklich: Standard Application, Single-Option, Group Registration, Supplementary Registration oder Recordation?
- Sind title, claimant, author, publication date, excluded material, AI-Anteile und deposit copy konsistent?
- Wird digital hochgeladen oder physisch mit Shipping Slip eingereicht, und warum ist die Alternative unzulässig oder riskant?
- Welche Frist-/Remedy-Wirkung hat timing nach § 411/§ 412 und was muss als Nachweis in die Verlagsakte?

**Mindest-Output:** Registrierungsakte mit Antragstyp, Werkdaten, Rechtekette, Deposit-Route, Fee/Tracking und Certificate-Archiv.

## Qualitäts- und Risikofilter

- US-Bundesrecht, State-Law-Claims und Vertragsrecht strikt trennen.
- Bei drohender US-Litigation fruehzeitig US-Counsel einschalten.

- Unklare US-Rechtsfragen werden nicht als sicher verkauft, sondern als Issue List für US-Counsel markiert.
- Zahlen, Fristen, Fees, Formulare und Schwellenwerte werden vor konkreter Verwendung anhand offizieller aktueller Quellen geprüft.

## 2. `office-action-rueckfragen-45-tage`

**Fokus:** Office Actions und Rückfragen: Monitoring, Antwortfristen, fehlende Angaben, Claim-Begrenzung, Nachlieferungen und Eskalation zu US-Counsel.

# Office Actions und Rückfragen

## Fachkern: Office Actions und Rückfragen
- **Spezialgegenstand:** Office Actions und Rückfragen. Die Prüfung setzt bei der konkreten Sachfrage an und endet mit einem verwertbaren Arbeitsergebnis.
- **Normen-/Quellenanker:** Title 17 U.S.C., Copyright Office Compendium, eCO-Verfahren, deposit/best edition, DMCA, fair use, termination, work made for hire und international treaties.
- **Entscheidende Weiche:** Werkart, Autor/Rechteinhaber, Veröffentlichung, Deposit, Claim/Exclusion, Registrierungsdatum, Enforcement-Ziel und US-Prozessnutzen trennen.
- **Arbeitsprodukt:** Erzeuge eine konkrete Prüf- oder Entscheidungsmatrix mit Norm, Tatbestand, Beleg, Einwand, Risikoampel und nächstem Schritt; Anschluss-Skills nur bei echter Vertiefung nennen.


## Aufgabe

Organisiere Rückfragen des U.S. Copyright Office so, dass sie nicht in einer allgemeinen Verlagsmailbox versanden. Der Skill baut aus einer Office Action eine klare Antwortakte.

## Prüfprogramm

1. Rückfrage vollständig erfassen: Datum, Case Number, Werk, Examiner, Antwortweg und Frist.
2. Einordnen: fehlende Werkdaten, unklarer Claimant, falscher Antragstyp, Deposit-Problem, AI-Material, fremde Inhalte, Veröffentlichungsstatus oder Rechtekette.
3. Belege zusammenstellen: Autorenvertrag, Rechteübertragung, Impressum, Publikationsnachweis, Deposit-Screenshot, Kuriertracking.
4. Antwort entwerfen: knapp, sachlich, mit eindeutiger Korrektur oder Erläuterung.
5. Eskalieren, wenn materielles US-Recht, Ownership-Dispute, Refusal-Risiko oder Litigation-Bezug sichtbar wird.

## Output

- Fristenblatt,
- Antwortentwurf,
- Belegmatrix,
- Entscheidungsvorlage: selbst beantworten oder US-Counsel einschalten,
- Abschlussvermerk nach Submission.

## Qualitätslinie

Keine kreative Nachbesserung am Sachverhalt. Wenn die ursprüngliche Anmeldung falsch war, offen korrigieren und Supplementary Registration oder Neuantrag prüfen.

## Quellenhygiene

Aktuelle Gebühren, Formulare, Deposit-Anforderungen, Gruppenoptionen und Bearbeitungszeiten vor einer konkreten Ausgabe immer auf den offiziellen Seiten des U.S. Copyright Office prüfen. Rechtliche Schlussfolgerungen zu US-Prozessführung, Ownership-Streit, Work made for hire, AI-Material oder Schadenersatz nur als Vorprüfung formulieren und bei Tragweite US-Counsel einschalten.

## Tiefencheck für die Akte

Arbeite hier nicht nur abstrakt, sondern als Rechte-, Werk- und Beweisfilter:

- Welcher Antragstyp passt wirklich: Standard Application, Single-Option, Group Registration, Supplementary Registration oder Recordation?
- Sind title, claimant, author, publication date, excluded material, AI-Anteile und deposit copy konsistent?
- Wird digital hochgeladen oder physisch mit Shipping Slip eingereicht, und warum ist die Alternative unzulässig oder riskant?
- Welche Frist-/Remedy-Wirkung hat timing nach § 411/§ 412 und was muss als Nachweis in die Verlagsakte?

**Mindest-Output:** Registrierungsakte mit Antragstyp, Werkdaten, Rechtekette, Deposit-Route, Fee/Tracking und Certificate-Archiv.

## 3. `online-subpoena-512h`

**Fokus:** Prueft Subpoena zur Identifizierung mutmaßlicher Verletzer, Notice-Voraussetzungen, Provider-Rolle und Datenschutzfolgen.

# DMCA Subpoena § 512h

## Fachkern: DMCA Subpoena § 512h
- **Spezialgegenstand:** DMCA Subpoena § 512h. Die Prüfung setzt bei der konkreten Sachfrage an und endet mit einem verwertbaren Arbeitsergebnis.
- **Normen-/Quellenanker:** Title 17 U.S.C., Copyright Office Compendium, eCO-Verfahren, deposit/best edition, DMCA, fair use, termination, work made for hire und international treaties.
- **Entscheidende Weiche:** Werkart, Autor/Rechteinhaber, Veröffentlichung, Deposit, Claim/Exclusion, Registrierungsdatum, Enforcement-Ziel und US-Prozessnutzen trennen.
- **Arbeitsprodukt:** Erzeuge eine konkrete Prüf- oder Entscheidungsmatrix mit Norm, Tatbestand, Beleg, Einwand, Risikoampel und nächstem Schritt; Anschluss-Skills nur bei echter Vertiefung nennen.


## Zweck

Das US-Copyright-Act-Plugin deckt Title 17 nicht nur als Registrierungsab, sondern als Praxiswerkzeug fuer Rechteinhaber, deutsche Verlage, Plattformen, Produzenten, Lizenznehmer und Prozessvorbereitung.

Dieser Skill macht aus dem Thema **DMCA Subpoena § 512h** eine belastbare Arbeitsroute: erst Rolle und Ziel, dann Code-Anker, Tatsachen, Dokumente, Risiken, offene US-Counsel-Fragen und verwertbarer Output. Er ist kein Ersatz für zugelassenen US-Counsel, aber er soll die deutsche Kanzlei, Rechtsabteilung oder den Verlag so vorbereiten, dass US-Counsel sofort mit einer geordneten Akte weiterarbeiten kann.

## Code- und Quellenanker

- 17 U.S.C. § 512(h)
- Federal clerk practice live prüfen
- Platform identity data


Aktuelle Fassungen, Forms, Fees, Local Rules und Court-Practice immer live über offizielle Quellen prüfen. Keine Modellzitate zu US-Rechtsprechung verwenden, wenn Gericht, Datum, Aktenzeichen/Docket und freie Quelle nicht geprüft sind.

## Intake-Fragen

- Welche Identität wird benötigt?
- Liegt korrekte takedown notice vor?
- Ist Provider covered und Daten vorhanden?


## Workflow

1. Notice package prüfen.
2. Subpoena request vorbereiten.
3. Datenschutz und foreign client implications markieren.


## Output

- Subpoena checklist
- Provider evidence memo
- US-counsel package


## Tiefencheck für die Akte

Arbeite hier nicht nur abstrakt, sondern als Rechte-, Werk- und Beweisfilter:

- Welche Rolle hat die Partei: rights owner, platform, host, search tool, user, agent oder reseller?
- Sind designated agent, policy, knowledge/red-flag, expeditious removal und counter-notice sauber dokumentiert?
- Ist die Notice spezifisch genug und enthält sie keine Überdehnung fremder Rechte oder falsche Claimant-Angaben?
- Welche Eskalation droht: restoration, subpoena, repeat-infringer issue, lawsuit hold oder platform suspension?

**Mindest-Output:** DMCA-mit Notice-Check, Safe-Harbor-Gap, Counter-Notice-Risiko und Evidence Hold.

## Qualitäts- und Risikofilter

- US-Bundesrecht, State-Law-Claims und Vertragsrecht strikt trennen.
- Bei drohender US-Litigation fruehzeitig US-Counsel einschalten.

- Unklare US-Rechtsfragen werden nicht als sicher verkauft, sondern als Issue List für US-Counsel markiert.
- Zahlen, Fristen, Fees, Formulare und Schwellenwerte werden vor konkreter Verwendung anhand offizieller aktueller Quellen geprüft.

## 4. `ownership-transfer-writing`

**Fokus:** Prueft Übertragung, exklusive Lizenz, nonexclusive license, writing requirement und chain of title.

# Ownership, Assignment und Writing Requirement

## Fachkern: Ownership, Assignment und Writing Requirement
- **Spezialgegenstand:** Ownership, Assignment und Writing Requirement. Die Prüfung setzt bei der konkreten Sachfrage an und endet mit einem verwertbaren Arbeitsergebnis.
- **Normen-/Quellenanker:** Title 17 U.S.C., Copyright Office Compendium, eCO-Verfahren, deposit/best edition, DMCA, fair use, termination, work made for hire und international treaties.
- **Entscheidende Weiche:** Werkart, Autor/Rechteinhaber, Veröffentlichung, Deposit, Claim/Exclusion, Registrierungsdatum, Enforcement-Ziel und US-Prozessnutzen trennen.
- **Arbeitsprodukt:** Erzeuge eine konkrete Prüf- oder Entscheidungsmatrix mit Norm, Tatbestand, Beleg, Einwand, Risikoampel und nächstem Schritt; Anschluss-Skills nur bei echter Vertiefung nennen.


## Zweck

Das US-Copyright-Act-Plugin deckt Title 17 nicht nur als Registrierungsab, sondern als Praxiswerkzeug fuer Rechteinhaber, deutsche Verlage, Plattformen, Produzenten, Lizenznehmer und Prozessvorbereitung.

Dieser Skill macht aus dem Thema **Ownership, Assignment und Writing Requirement** eine belastbare Arbeitsroute: erst Rolle und Ziel, dann Code-Anker, Tatsachen, Dokumente, Risiken, offene US-Counsel-Fragen und verwertbarer Output. Er ist kein Ersatz für zugelassenen US-Counsel, aber er soll die deutsche Kanzlei, Rechtsabteilung oder den Verlag so vorbereiten, dass US-Counsel sofort mit einer geordneten Akte weiterarbeiten kann.

## Code- und Quellenanker

- 17 U.S.C. § 201
- 17 U.S.C. § 204
- 17 U.S.C. § 205


Aktuelle Fassungen, Forms, Fees, Local Rules und Court-Practice immer live über offizielle Quellen prüfen. Keine Modellzitate zu US-Rechtsprechung verwenden, wenn Gericht, Datum, Aktenzeichen/Docket und freie Quelle nicht geprüft sind.

## Intake-Fragen

- Ist die Lizenz exklusiv oder nicht exklusiv?
- Gibt es schriftliche Unterschrift des Rechteinhabers?
- Welche Rechte, Territorien, Medien und Dauer?


## Workflow

1. Transferdokumente sammeln.
2. Scope gegen Nutzung abgleichen.
3. Recordation und priority prüfen.


## Output

- Chain-of-title memo
- Transfer gap list
- Recordation recommendation


## Tiefencheck für die Akte

Arbeite hier nicht nur abstrakt, sondern als Rechte-, Werk- und Beweisfilter:

- Wer ist author, claimant, owner, assignee, employer, commissioned party, joint author oder secured party?
- Gibt es eine schriftliche Übertragung, Work-made-for-hire-Logik, deutsches Vertragsrecht oder nur operative Gewohnheit?
- Drohen termination rights, priority conflicts, unrecorded transfers, security interests oder chain-of-title gaps?
- Welche Dokumente muss der Verlag sammeln: Verträge, Nachträge, invoices, acceptance, employment records, board approvals?

**Mindest-Output:** Chain-of-Title-Gutachten mit Rollen, Dokumenten, Lücken, Recordation-Option und Deal-Risiko.

## Qualitäts- und Risikofilter

- US-Bundesrecht, State-Law-Claims und Vertragsrecht strikt trennen.
- Bei drohender US-Litigation fruehzeitig US-Counsel einschalten.

- Unklare US-Rechtsfragen werden nicht als sicher verkauft, sondern als Issue List für US-Counsel markiert.
- Zahlen, Fristen, Fees, Formulare und Schwellenwerte werden vor konkreter Verwendung anhand offizieller aktueller Quellen geprüft.
