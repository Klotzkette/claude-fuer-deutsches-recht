# Akte: Korruptionsverdacht in der Vergabestelle München

<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

[Testakten-Übersicht](../README.md) · [Repository-Start](../../README.md) · [Plugin-Katalog](../../README.md#was-ist-drin) · [Download-Index](../../ASSET_INDEX.md)

Dieses Aktenpaket gibt es in drei Formaten. Das Gesamt-PDF eignet sich zum Lesen und Ausdrucken. Das Akten-ZIP enthält die nativen Originaldateien wie DOCX, Tabellen, E-Mails, Fotos und PDFs. Es enthält kein Markdown; sämtliche Dateien liegen ohne Unterordner unmittelbar auf der ZIP-Wurzelebene. Das Einzel-PDF-ZIP liefert jede Unterlage als separate, sauber gerenderte PDF unmittelbar auf der ZIP-Wurzelebene.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei) | PDF | [`gesamt-pdf/strafrecht-korruption-bauamt-vergabestelle-muenchen_gesamt.pdf`](gesamt-pdf/strafrecht-korruption-bauamt-vergabestelle-muenchen_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-strafrecht-korruption-bauamt-vergabestelle-muenchen.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-strafrecht-korruption-bauamt-vergabestelle-muenchen.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-strafrecht-korruption-bauamt-vergabestelle-muenchen-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-strafrecht-korruption-bauamt-vergabestelle-muenchen-einzelpdfs.zip) |

Die ZIP-Links laden den zuletzt veröffentlichten Release. Das Gesamt-PDF ist auch im Akten-ZIP enthalten; für eine einheitliche Arbeitsfassung genügt deshalb dieses Archiv. Der hier verlinkte Repository-Stand kann zwischen Releases bereits neuer sein.

English: The original-format ZIP contains the working files directly at archive root, without subfolders or Markdown. Choose the combined PDF for reading; it is also included in that ZIP. Choose the individual-PDF ZIP to review each document separately. These are practice documents, not an installable plugin. ZIP links refer to the latest published release.

<!-- END gesamt-pdf-section (autogen) -->

## Sachverhalt

Der Stadtoberinspektor Markus Eberl bearbeitete Rahmenverträge für Straßenunterhalt und Bauhofmaterial. Die Firma Kiesel & Rath GmbH erhielt in drei Vergaben jeweils den Zuschlag knapp unterhalb interner Kontrollschwellen. Parallel fanden Einladungen zu Fußballspielen und ein Wochenende in Südtirol statt, bezahlt durch den Vertriebsleiter der Firma.

Eberl behauptet, die Einladungen seien private Freundschaftspflege gewesen und er habe keine Wertungspunkte manipuliert. Die Vergabeakte zeigt aber ungewöhnliche Loszuschnitte, nachträgliche Bewertungsvermerke und eine E-Mail, in der der Vertriebsleiter die Formulierung eines Leistungsmerkmals vorschlägt. Ein Konkurrenzunternehmen rügt die Ausschreibung wegen Produktnähe.

Die Akte verbindet Strafrecht, Vergaberecht, Disziplinarrecht und Compliance. Entscheidend sind Diensthandlung, Unrechtsvereinbarung, Vorteilshöhe, Kausalität für den Zuschlag und die Frage, ob eine belastbare Schadensberechnung möglich ist.

Der Beweiskern liegt im zeitlichen Abgleich der drei Zuschläge an die Kiesel und Rath GmbH mit den Zuwendungen an den Beschuldigten. Die Datei `10_vergabe_und_vorteile_abgleich.csv` führt Zuschlagsdaten, Auftragswerte, Kalendereinträge und Kontobewegungen zusammen; erst die Verbindung mit dem Auswertevermerk, dem WhatsApp-Verlauf und der Beschuldigtenvernehmung macht das Muster (Zuschlag, wenige Tage später Vorteil) und die als "Beratungshonorar" getarnte Zahlung sichtbar.

## Verfahrensstand

Gericht: Landgericht München I, Wirtschaftsstrafkammer 6

Staatsanwaltschaft: Staatsanwaltschaft München I, 6 Js 9940/26

Verteidigung oder Vertretung: RA Dr. Livia Sauter, München

Mandatsbezug: Markus Eberl, Stadtoberinspektor in der Vergabestelle

## Beteiligte

| Name | Rolle |
| --- | --- |
| Markus Eberl | Beschuldigter, Stadtoberinspektor in der Zentralen Vergabestelle |
| Ferdinand Kolb | Vertriebsleiter der Kiesel und Rath GmbH, Zuwendungsgeber |
| Kiesel und Rath GmbH | begünstigte Bieterin in allen drei Vergaben |
| BauTec Süd GmbH | unterlegene Konkurrentin, rügt Produktnähe der Ausschreibung |
| Dr. Sabine Ortner | Antikorruptionsbeauftragte der Landeshauptstadt München |
| Hans-Georg Wenzl | Leitender Baudirektor, Amtsleitung Baureferat |
| RA Dr. Livia Sauter | Verteidigerin des Beschuldigten |

## Zeitachse

| Datum | Vorgang |
| --- | --- |
| 14.02.2026 | interne Compliance-Meldung über auffällige Loszuschnitte |
| 03.03.2026 | Durchsuchung Vergabestelle und Privatwohnung |
| 17.03.2026 | Sicherstellung Diensthandy und privater Kalender |
| 07.05.2026 | Vergabevermerke und Wertungsmatrix in Akte |
| 28.06.2026 | Anhörung Beamtenrecht angekündigt |
| 16.09.2026 | möglicher Termin Haftungs- und Disziplinargespräch |

## Aktenstruktur

```
strafrecht-korruption-bauamt-vergabestelle-muenchen/
├── 02_sachverhalt_chronologie.docx                     Ausformulierter Sachverhalt, Zeitachse und offene Widersprüche
├── 03_ermittlungsakte_auszuege.docx                    Auszüge aus polizeilichen und staatsanwaltlichen Vermerken
├── 04_beweismittel_und_arbeitsauftraege.xlsx           Beweismittelmatrix mit Belastungs- und Entlastungsrichtungen
├── 05_fristen_und_verfahrensstand.csv                  Fristen, Termine, Zustellungen und Wiedervorlagen
├── 06_email_akteneinsicht_und_rueckfragen.eml          E-Mail zu Akteneinsicht und Nachforderungen
├── 07_rechtliche_arbeitsnotiz.docx                     Normanker, Beweislastfragen, taktische Linien
├── 08_entwurf_verfahrensschritt.docx                   Entwurf für Stellungnahme, Antrag oder Rechtsmittel
├── 09_originalanlage_behoerdenvermerk.pdf              PDF-Anlage mit behördlichem oder fachlichem Vermerk
├── 10_vergabe_und_vorteile_abgleich.csv               Zuschlagsliste gegen Zuwendungen und Kontobewegungen (Datenauswertungs-Kern)
├── 11_beschuldigtenvernehmung_eberl_2026-06-18.docx    Einlassung Eberl: Freundschaft und "Beratungshonorar"
├── eml/
│   ├── 01_compliance_meldung_loszuschnitte_2026-02-14.eml  Compliance-Meldung über auffällige Loszuschnitte an die Amtsleitung
│   ├── 02_kolb_leistungsmerkmal_2025-10-08.eml            Kolb schlägt Leistungsmerkmal vor und kündigt Honorar an
│   └── 03_anonyme_anzeige_2026-02-09.eml                  Anonymer Hinweis an die Zentrale Anlaufstelle Korruption
├── whatsapp/
│   └── chatverlauf.txt                                  Chat Eberl/Kolb: Zuschläge, Loge, Südtirol und "2.500 nach dem Zuschlag"
├── README.md                                            Diese Übersicht
└── rubric.yaml                                          Prüfpunkte für die Bearbeitung der Akte
```

## Prüffokus

- Bestechlichkeit nach Paragraf 332 StGB
- Vorteilsannahme nach Paragraf 331 StGB
- Untreueprüfung wegen Vergabeschaden
- Beweiswert und Beweisverwertungsfragen getrennt prüfen.
- Nebenfolgen, Fristen und Verfahrensziel früh sichtbar machen.

## Passende Arbeitsrichtungen

`fachanwalt-strafrecht`, `fachanwalt-vergaberecht`, `staatsanwaltschaft-praxis-einstieg`

---

<!-- BEGIN nachgereichte-originalunterlagen -->
## Nachgereichte Originalunterlagen

Diese Dateien ergänzen den Datenraum um später eingegangene Primärunterlagen. Sie enthalten Tatsachen, Rohdaten und Korrespondenz, aber keine rechtliche Lösung.

| Datei | Herkunft und Inhalt |
| --- | --- |
| [`16_vergabeakte_eingang_angebote_und_bewirtung.docx`](16_vergabeakte_eingang_angebote_und_bewirtung.docx) | Vergabeaktenvermerk zu Dateien, Terminen und Bewertungsständen |
| [`17_email_bieter_rechnung_mittagessen.eml`](17_email_bieter_rechnung_mittagessen.eml) | Originale interne Bietermail zum unvollständigen Bewirtungsbeleg |
| [`18_angebote_bewertung_versionen_roh.csv`](18_angebote_bewertung_versionen_roh.csv) | Angebots- und Matrixversionen ohne vorweggenommene Würdigung |
| [`19_kontoauszug_eberl_oktober_november_2025.csv`](19_kontoauszug_eberl_oktober_november_2025.csv) | Kontoumsätze Eberl mit Gutschrift Beratungshonorar und Weiterleitung aufs Sparkonto |
| [`20_anhoerung_baureferat_disziplinarverfahren_2026-06-28.docx`](20_anhoerung_baureferat_disziplinarverfahren_2026-06-28.docx) | Anhörungsschreiben des Baureferats zur Einleitung und Aussetzung des Disziplinarverfahrens |

<!-- END nachgereichte-originalunterlagen -->
