# Teilhabeassistenz Berufsschule Regensburg: Autismus, Schulweg und Kommunikationshilfen

<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

[Testakten-Übersicht](../README.md) · [Repository-Start](../../README.md) · [Plugin-Katalog](../../README.md#was-ist-drin) · [Download-Index](../../ASSET_INDEX.md)

Dieses Aktenpaket gibt es in drei Formaten. Das Gesamt-PDF eignet sich zum Lesen und Ausdrucken. Das Akten-ZIP enthält die nativen Originaldateien wie DOCX, Tabellen, E-Mails, Fotos und PDFs. Es enthält kein Markdown; sämtliche Dateien liegen ohne Unterordner unmittelbar auf der ZIP-Wurzelebene. Das Einzel-PDF-ZIP liefert jede Unterlage als separate, sauber gerenderte PDF unmittelbar auf der ZIP-Wurzelebene.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei) | PDF | [`gesamt-pdf/sozialrecht-teilhabeassistenz-autismus-berufsschule-regensburg_gesamt.pdf`](gesamt-pdf/sozialrecht-teilhabeassistenz-autismus-berufsschule-regensburg_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-sozialrecht-teilhabeassistenz-autismus-berufsschule-regensburg.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-sozialrecht-teilhabeassistenz-autismus-berufsschule-regensburg.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-sozialrecht-teilhabeassistenz-autismus-berufsschule-regensburg-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-sozialrecht-teilhabeassistenz-autismus-berufsschule-regensburg-einzelpdfs.zip) |

Die ZIP-Links laden den zuletzt veröffentlichten Release. Das Gesamt-PDF ist auch im Akten-ZIP enthalten; für eine einheitliche Arbeitsfassung genügt deshalb dieses Archiv. Der hier verlinkte Repository-Stand kann zwischen Releases bereits neuer sein.

English: The original-format ZIP contains the working files directly at archive root, without subfolders or Markdown. Choose the combined PDF for reading; it is also included in that ZIP. Choose the individual-PDF ZIP to review each document separately. These are practice documents, not an installable plugin. ZIP links refer to the latest published release.

<!-- END gesamt-pdf-section (autogen) -->

Der 19-jährige Auszubildende Levin Schuster braucht für Blockunterricht, Prüfungsvorbereitung und Schulweg eine Assistenz. Der Träger trennt schulische Eingliederungshilfe, Berufsausbildungsbeihilfe und Fahrdienst so eng, dass am Ende niemand zahlen will.

## Akteninhalt

1. Ausgangslage, Bescheide, Schreiben oder Registerauszüge.
2. Medizinische, wirtschaftliche oder markenrechtliche Belege mit Fristenkern.
3. Kommunikation, Tabellen und Arbeitsfassungen für Widerspruch, Klage, Vergleich oder Prüfungstermin.

## Geeignete Plugins

`fachanwalt-sozialrecht`, `teilhaberecht-rehabilitation`, `schulrecht-laender`, `selbstvertreter-sozialgericht`.

## Sachverhaltskern

Levin Schuster absolviert eine Ausbildung zum Fachlageristen bei der Donauhafen Logistik GmbH. Im Betrieb funktioniert die Struktur mit festem Ansprechpartner, in der Berufsschule brechen Stundenwechsel, Raumänderungen und Pausenhofsituationen regelmäßig zusammen. Der Bezirk verweist auf die Agentur für Arbeit, die Agentur auf die Schule, die Schule auf den Ausbildungsbetrieb. Die schriftliche Abschlussprüfung ist im Januar 2027 geplant; die laufende Blockphase beginnt am 14.09.2026. Die Mutter hat den Widerspruch bereits eingelegt, aber nur mit der Formulierung, Levin könne ohne Hilfe nicht teilnehmen. Die Akte enthält Förderplan, fachärztliche Diagnose, Ausbildungsbeurteilung und ein Gesprächsprotokoll der Schule.

---

<!-- BEGIN nachgereichte-originalunterlagen -->
## Nachgereichte Originalunterlagen

Diese Dateien ergänzen den Datenraum um später eingegangene Primärunterlagen. Sie enthalten Tatsachen, Rohdaten und Korrespondenz, aber keine rechtliche Lösung.

| Datei | Herkunft und Inhalt |
| --- | --- |
| [`08_telefonvermerk_agentur_fuer_arbeit_2026-07-09.docx`](08_telefonvermerk_agentur_fuer_arbeit_2026-07-09.docx) | Telefonvermerk der Kanzlei mit dem Reha-Team der Agentur für Arbeit zu Antragslage und Fallkonferenz |
| [`09_sms_verlauf_ulrike_levin_april_2026.txt`](09_sms_verlauf_ulrike_levin_april_2026.txt) | SMS-Auszug Mutter und Sohn aus der April-Blockwoche mit dem Vorfall an der Bushaltestelle |
| [`eml/2026-07-14_assistenzdienst_kapazitaet_september.eml`](eml/2026-07-14_assistenzdienst_kapazitaet_september.eml) | Angebot eines Assistenzdienstes mit Fachkraft, Stundensatz und Fristsetzung für die Kostenzusage |

<!-- END nachgereichte-originalunterlagen -->
