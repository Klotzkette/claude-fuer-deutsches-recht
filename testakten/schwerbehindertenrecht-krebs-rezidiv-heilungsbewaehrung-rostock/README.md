# Schwerbehindertenrechtsakte Krebsrezidiv Heilungsbewährung Rostock

<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

[Testakten-Übersicht](../README.md) · [Repository-Start](../../README.md) · [Plugin-Katalog](../../README.md#was-ist-drin) · [Download-Index](../../ASSET_INDEX.md)

Dieses Aktenpaket gibt es in drei Formaten zum Direkt-Download. Das Gesamt-PDF eignet sich zum Lesen, Ausdrucken und für schnelle Durchsichten. Das Akten-ZIP enthält die nativen Originaldateien wie DOCX, Tabellen, E-Mails, Fotos und PDFs. Es enthält kein Markdown; sämtliche Dateien liegen ohne Unterordner unmittelbar auf der ZIP-Wurzelebene. Das Einzel-PDF-ZIP liefert jede Unterlage als separate, sauber gerenderte PDF unmittelbar auf der ZIP-Wurzelebene.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei) | PDF | [`gesamt-pdf/schwerbehindertenrecht-krebs-rezidiv-heilungsbewaehrung-rostock_gesamt.pdf`](gesamt-pdf/schwerbehindertenrecht-krebs-rezidiv-heilungsbewaehrung-rostock_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-schwerbehindertenrecht-krebs-rezidiv-heilungsbewaehrung-rostock.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-schwerbehindertenrecht-krebs-rezidiv-heilungsbewaehrung-rostock.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-schwerbehindertenrecht-krebs-rezidiv-heilungsbewaehrung-rostock-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-schwerbehindertenrecht-krebs-rezidiv-heilungsbewaehrung-rostock-einzelpdfs.zip) |

Die ZIP-URLs sind stabil und zeigen immer auf die aktuelle Version. Im Akten-ZIP ist das Gesamt-PDF mit enthalten.

<!-- END gesamt-pdf-section (autogen) -->

Arbeitsakte zu GdB-Neufeststellung nach Brustkrebsrezidiv, Lymphödem, Fatigue und Streit um Heilungsbewährung. Die Unterlagen verbinden Onkologie, Bescheid, Widerspruch, Arbeitsplatzfolgen und rentenrechtliche Prüfung für `fachanwalt-sozialrecht` und `rentenpruefer`.

Enthalten sind ausformulierte Aktenstücke, Bescheide, medizinische oder betriebliche Unterlagen, Fristenmaterial, Tabellen und Arbeitsentwürfe. Die rechtliche Bewertung bleibt offen; die Unterlagen liefern den Sachverhalt für Widerspruch, Klage, Eilverfahren, Statusprüfung, Reha- und Rentenfolgefragen.

## Kurzbild

Doreen Kahl, geboren am 30.05.1976, Verkäuferin in einer Buchhandlung in Rostock, war 2019 an einem Mammakarzinom links erkrankt; der Grad der Behinderung war damals mit 50 befristet festgestellt und nach Ablauf der fünfjährigen Heilungsbewährung 2024 auf 30 herabgesetzt worden. Seit Februar 2026 besteht ein lokal-regionales Rezidiv mit Befall axillärer Lymphknoten. Nach erneuter operativer Entfernung am 04.04.2026 läuft seit dem 12.05.2026 eine Chemotherapie; Bestrahlung und endokrine Therapie sind angeschlossen. Auf den Änderungsantrag vom 18.03.2026 hat das Landesamt für Gesundheit und Soziales Mecklenburg-Vorpommern mit Bescheid vom 16.06.2026 nur einen Gesamt-GdB von 40 festgestellt und die Schwerbehinderteneigenschaft verweigert. Der Streit dreht sich um die erneute Heilungsbewährung: Mit der Geschwulstentfernung am 04.04.2026 beginnt sozialmedizinisch eine neue, bis zum 04.04.2031 laufende Heilungsbewährung, während der ein Grad der Behinderung von mindestens 50 anzusetzen ist. Die Behörde verwendet die laufende Therapie zugleich als Anerkennungsgrund und als Begrenzung.

## Beteiligte

| Person oder Stelle | Rolle |
| --- | --- |
| Doreen Kahl | Mandantin, Antragstellerin |
| Uwe Brenner | Rentenberater, Bevollmächtigter im Widerspruchsverfahren |
| Landesamt für Gesundheit und Soziales Mecklenburg-Vorpommern | Feststellungs- und Widerspruchsbehörde |
| Dr. med. R. Petermann | versorgungsärztliche Stelle des Landesamts |
| Onkologisches Zentrum Rostock, Dr. med. Sabine Voigt | behandelndes Tumorzentrum |
| Anne Kahl | Schwester, Begleitperson zu den Chemotherapien |

## Aktenstruktur

```
schwerbehindertenrecht-krebs-rezidiv-heilungsbewaehrung-rostock/
├── README.md                                   <- diese Übersicht
├── 02_onkologiebericht.docx                     <- Diagnose, Funktionsfolgen, Prognose
├── 03_bescheid_gdb_40.docx                      <- Feststellungsbescheid mit Einzel-GdB und Begründung
├── 04_therapiekalender.csv                      <- Behandlungstermine, Folgen, Arbeitsausfall
├── 05_arbeitgebernotiz_buchhandlung.docx        <- Tätigkeit, Einschränkungen, Teilhabefolgen
├── 06_widerspruch_entwurf.docx                  <- Widerspruch mit Antrag auf Schwerbehinderteneigenschaft
├── 07_renten_reha_krankengeld.docx              <- Krankengeld, Reha, Rentenabgrenzung
├── 08_email_schwester.eml                       <- Schwester zur Ambivalenz und Begleitung
├── 09_gdb_heilungsbewaehrung_zeitleiste.csv     <- Datenkern: GdB-Verlauf, Heilungsbewährung, Nachprüfung
├── 10_versorgungsaerztliche_stellungnahme.docx  <- interne Stellungnahme mit Gesamt-GdB 40
├── 11_tumorzentrum_stellungnahme_heilungsbewaehrung.docx <- onkologische Stellungnahme zur neuen Heilungsbewährung
├── 12_chat_schwester.txt                         <- Chat der Schwestern zu Bescheid und Heilungsbewährung
├── eml/
│   ├── 01_landesamt_eingangsbestaetigung.eml     <- Eingangsbestätigung, Nachfrage OP- und Therapiedaten
│   ├── 02_tumorzentrum_befunduebersendung.eml     <- Tumorzentrum übersendet Befund und Stellungnahme
│   └── 03_bevollmaechtigter_heilungsbewaehrung.eml <- Bevollmächtigter legt die erneute Heilungsbewährung dar
└── gesamt-pdf/                                   <- konsolidierte Lesefassung als PDF
```

## Bearbeitungsziel

Aus der Zeitleiste, dem Onkologiebericht und den Stellungnahmen ist der GdB-Verlauf nachzurechnen: Erstdiagnose 2019 mit Heilungsbewährung bis 2024 (GdB 50), Herabsetzung auf 30, Rezidiv 2026 mit erneuter Geschwulstentfernung am 04.04.2026 und dadurch neuer, bis 04.04.2031 laufender Heilungsbewährung. Herauszuarbeiten ist, dass während dieser Heilungsbewährung ein Grad der Behinderung von mindestens 50 maßgeblich ist und damit die Schwerbehinderteneigenschaft besteht, während der Bescheid bei 40 verharrt. Die Diskrepanz zwischen dem Bescheidwert 40 und dem Heilungsbewährungssatz 50 sowie der widersprüchliche Umgang der Behörde mit der laufenden Therapie gehören zur Beweiswürdigung. Parallel sind Krankengeld, onkologische Reha und die Rentenabgrenzung zu sichern.

---

<!-- BEGIN nachgereichte-originalunterlagen -->
## Nachgereichte Originalunterlagen

Diese Dateien ergänzen den Datenraum um später eingegangene Primärunterlagen. Sie enthalten Tatsachen, Rohdaten und Korrespondenz, aber keine rechtliche Lösung.

| Datei | Herkunft und Inhalt |
| --- | --- |
| [`16_onkologischer_verlaufsbericht_rezidiv.docx`](16_onkologischer_verlaufsbericht_rezidiv.docx) | Onkologischer Verlauf mit messbaren Funktionsbeeinträchtigungen |
| [`17_email_arbeitgeber_fehlzeiten_anpassung.eml`](17_email_arbeitgeber_fehlzeiten_anpassung.eml) | Arbeitgebermail zur tatsächlich gelebten Arbeitsplatzanpassung |
| [`18_therapie_lymphoedem_und_belastung.csv`](18_therapie_lymphoedem_und_belastung.csv) | Therapie-, Mess- und Fehlzeitendaten |
| [`19_telefonvermerk_brenner_lagus.md`](19_telefonvermerk_brenner_lagus.md) | Telefonvermerk des Bevollmächtigten zu Aktenlage und Operationsdatum |
| [`eml/04_lagus_zwischennachricht_widerspruch.eml`](eml/04_lagus_zwischennachricht_widerspruch.eml) | Zwischennachricht der Widerspruchsstelle mit Verfahrensstand und Fristen |

<!-- END nachgereichte-originalunterlagen -->
