# Akte: Wirtschaftsstrafsache Bankert — U-Haft, Betrug, Steuerhinterziehung, LG Frankfurt

<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

[Testakten-Übersicht](../README.md) · [Repository-Start](../../README.md) · [Plugin-Katalog](../../README.md#was-ist-drin) · [Download-Index](../../ASSET_INDEX.md)

Dieses Aktenpaket gibt es in drei Formaten. Das Gesamt-PDF eignet sich zum Lesen und Ausdrucken. Das Akten-ZIP enthält die nativen Originaldateien wie DOCX, Tabellen, E-Mails, Fotos und PDFs. Es enthält kein Markdown; sämtliche Dateien liegen ohne Unterordner unmittelbar auf der ZIP-Wurzelebene. Das Einzel-PDF-ZIP liefert jede Unterlage als separate, sauber gerenderte PDF unmittelbar auf der ZIP-Wurzelebene.

> Diese Testakte wurde mit KI generiert und ist ein Experiment. Benutzung auf eigene Verantwortung und eigene Gefahr.
>
> This test case file was generated with AI and is an experiment. Use at your own responsibility and risk.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei) | PDF | [`gesamt-pdf/wirtschaftsstrafsache-uhaft-bankert-frankfurt_gesamt.pdf`](gesamt-pdf/wirtschaftsstrafsache-uhaft-bankert-frankfurt_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-wirtschaftsstrafsache-uhaft-bankert-frankfurt.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-wirtschaftsstrafsache-uhaft-bankert-frankfurt.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-wirtschaftsstrafsache-uhaft-bankert-frankfurt-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-wirtschaftsstrafsache-uhaft-bankert-frankfurt-einzelpdfs.zip) |

Die ZIP-Links laden den zuletzt veröffentlichten Release. Das Gesamt-PDF ist auch im Akten-ZIP enthalten; für eine einheitliche Arbeitsfassung genügt deshalb dieses Archiv. Der hier verlinkte Repository-Stand kann zwischen Releases bereits neuer sein.

English: The original-format ZIP contains the working files directly at archive root, without subfolders or Markdown. Choose the combined PDF for reading; it is also included in that ZIP. Choose the individual-PDF ZIP to review each document separately. These are practice documents, not an installable plugin. ZIP links refer to the latest published release.

<!-- END gesamt-pdf-section (autogen) -->

Arbeitsakte zum Plugin `fachanwalt-strafrecht`.

---

## Kurzbild

- **Beschuldigter:** Daniel Albert Bankert, 46, Geschäftsführer der Bankert Industrieautomation GmbH, Frankfurt am Main.
- **Vorwürfe (Anklageschrift StA Frankfurt, Az. 5400 Js 14782/24, vom 08.04.2025):**
  - Gewerbsmäßiger Betrug § 263 Abs. 3 StGB: 14 Fälle, Schaden ca. 4.300.000 EUR (falsche Angaben über Industriemaschinen ggü. ABC Großabnehmer SE)
  - Untreue § 266 StGB: Gesellschaftsmittel ca. 870.000 EUR in Liechtenstein-Trust verschoben
  - Bestechung § 334 StGB: drei Einkäufer der ABC Großabnehmer SE bestochen
  - Steuerhinterziehung § 370 AO: VZ 2019–2022, ca. 1.100.000 EUR KSt/GewSt/USt verkürzt
- **U-Haft:** Bankert sitzt seit 17.03.2025 in der JVA Frankfurt I; Haftgrund Fluchtgefahr (zweite Ehefrau in Zürich, Auslandsvermögen Liechtenstein/Luxemburg).
- **Verteidigerin:** Dr. Friederike Marx, Fachanwältin für Strafrecht, Marx Strafverteidigung Frankfurt, Bockenheimer Landstraße 84, 60323 Frankfurt am Main.
- **Gericht:** LG Frankfurt am Main, 14. Große Wirtschaftsstrafkammer, Az. **5/28 KLs 122/25**. Hauptverhandlung ab 23.06.2025.
- **Nebenklägerin:** ABC Großabnehmer SE (RA Krämer, Krämer Hauck); Adäsionsantrag auf 3.800.000 EUR.
- **Verständigung § 257c StPO:** StA bietet an: Geständnis Betrug + Steuerhinterziehung ↔ Einstellung Untreue/Bestechung + Strafe 3 J. 9 M. Entscheidung ausstehend.
- **Brisante Punkte:** Widersprüche Belastungszeugen Pollmann/Kohlrausch; Durchsuchung Kanzleiräume Marx (§ 53/97 StPO-Widerspruch); Treuhandvertrag Anlage B14 (Echtheitsfrage); Insolvenzgefahr BIA GmbH.
- **Stichtag dieser Akte:** Mai 2025.

---

## Was diese Akte demonstriert

| Skill | Aktenstück(e) | Demonstration |
|---|---|---|
| Akteneinsicht beantragen | 05, 17 | § 147 StPO, vollständige Aktenein­sicht in 14 Bände, Modalitäten |
| Akteneinsicht auswerten | 06, 07 | Systematische Widerspruchs­analyse Zeugenaussagen (Pollmann, Kohlrausch) |
| Haftbeschwerde / Haftprüfung | 02, 12, 04 | § 112 Abs. 2 Nr. 2 StPO Flucht­gefahr, § 116 StPO mildere Mittel, § 304 StPO |
| § 53 StPO / Anwaltsgeheimnis | 09, 10 | Durchsuchung Kanzleiräume, § 97 StPO Beschlagnahmeverbot, BVerfG-Linie |
| Verständigung § 257c StPO | 11, 21 | Verständigungsparameter, Risiko-Kalkül, Gegenvorschlag |
| Beweisantrag | 08 | § 244 Abs. 3 StPO, Gegengutachten Maschinen­bewertung |
| Nebenklage / Adäsion | 13, 20 | §§ 395, 403 ff. StPO, Zulässigkeit, Gegenstrategie, Zivilvergleich |
| Zeugenauswertung | 06, 07 | Vier Kern-Widersprüche pro Zeuge, rechtliche Ansätze |
| Einlassung des Angeklagten | 19 | Vollständige Einlassung zu allen Anklagepunkten |
| Steuerhinterziehung § 370 AO | 14 | Betragsberechnung VZ 2019–2022, besonders schwerer Fall, Steuerberater-Verantwortlichkeit |
| Untreue / Trust-Konstrukt | 15 | § 266 StGB, Treuhandvertrag, Schriftvergleichsgutachten, Gesellschafterstreit |
| Mandanten­kommunikation | 04, 16 | Haftbesuchsnotiz, Mandantenbrief JVA, § 148 StPO |
| Strategiekonferenz | 21 | Vollständige Verteidigungsstrategie, Best/Worst-Case, Fristmanagement |
| Fristenkalender | 22 | HV-Terminplan, prozessuale Fristen, § 257c-Dokumentations­pflichten |
| Schadensberechnung Betrug | 20 (XLSX) | Schadenstabelle 14 Tatkomplexe, Anklage vs. Verteidigung |

---

## Aktenstücke

| Nr. | Datei | Inhalt |
|---|---|---|
| 01 | [`01_aktenvorblatt_marx_strafverteidigung.docx`](01_aktenvorblatt_marx_strafverteidigung.docx) | Aktenvorblatt, Mandatsdaten, Tatvorwürfe-Überblick, Verfahrensstand, Fristen |
| 02 | [`02_haftbefehl_lg_frankfurt_28_ks_122_25.docx`](02_haftbefehl_lg_frankfurt_28_ks_122_25.docx) | Haftbefehl LG Frankfurt (Auszug, redigiert) — Tatvorwürfe, Haftgründe, Vollzug |
| 03 | [`03_anklageschrift_staatsanwaltschaft_auszug.docx`](03_anklageschrift_staatsanwaltschaft_auszug.docx) | Anklageschrift StA Frankfurt (Auszug) — Wesentliches Ergebnis der Ermittlungen |
| 04 | [`04_kanzleinotiz_haftbesuch_jva_frankfurt_i.docx`](04_kanzleinotiz_haftbesuch_jva_frankfurt_i.docx) | Kanzleinotiz Haftbesuch JVA Frankfurt I, 28.04.2025 — Mandantenangaben, Strategie |
| 05 | [`05_akteneinsichts_antrag_marx.docx`](05_akteneinsichts_antrag_marx.docx) | Antrag auf Akteneinsicht § 147 StPO, Schriftsatz Dr. Marx an StA Frankfurt |
| 06 | [`06_zeugenaussage_pollmann_widersprueche.docx`](06_zeugenaussage_pollmann_widersprueche.docx) | Auswertung Zeuge Pollmann — vier dokumentierte Widersprüche in V1/V2/V3 |
| 07 | [`07_zeugenaussage_kohlrausch_widersprueche.docx`](07_zeugenaussage_kohlrausch_widersprueche.docx) | Auswertung Zeugin Kohlrausch — Motivlage, Widersprüche, Eigenstrafbarkeit |
| 08 | [`08_beweisantrag_sachverstaendiger_maschinenbewertung.docx`](08_beweisantrag_sachverstaendiger_maschinenbewertung.docx) | Beweisantrag § 244 Abs. 3 StPO auf Gegengutachten Maschinenbewertung |
| 09 | [`09_durchsuchungsbeschluss_kanzleiraeume.docx`](09_durchsuchungsbeschluss_kanzleiraeume.docx) | Durchsuchungsbeschluss AG Frankfurt, 10.03.2025 — Kanzleiräume Marx |
| 10 | [`10_widerspruch_mandatsdaten_53_stpo.docx`](10_widerspruch_mandatsdaten_53_stpo.docx) | Widerspruch gegen Sicherstellung — § 97, § 148 StPO, BVerfG-Linie |
| 11 | [`11_verstaendigungsangebot_staatsanwaltschaft.docx`](11_verstaendigungsangebot_staatsanwaltschaft.docx) | Verständigungsangebot StA § 257c StPO + interner Bewertungsvermerk Dr. Marx |
| 12 | [`12_haftbeschwerde_117_stpo.docx`](12_haftbeschwerde_117_stpo.docx) | Haftbeschwerde §§ 304, 117 StPO — vollständiger Schriftsatz, § 116-Antrag |
| 13 | [`13_nebenklage_abc_grossabnehmer.docx`](13_nebenklage_abc_grossabnehmer.docx) | Nebenklage ABC Großabnehmer SE — §§ 395, 396 StPO, Zulässigkeit, Strategie |
| 14 | [`14_steuerhinterziehung_betragsberechnung.docx`](14_steuerhinterziehung_betragsberechnung.docx) | § 370 AO — Berechnung VZ 2019–2022, Anklage vs. Verteidigung, § 371 AO |
| 15 | [`15_treuhandvertrag_liechtenstein_anlage_b14.docx`](15_treuhandvertrag_liechtenstein_anlage_b14.docx) | Treuhandvertrag Anlage B14 — Echtheitsfrage, Gegengutachten, § 266 StGB |
| 16 | [`16_mandantenbrief_marx_an_bankert.docx`](16_mandantenbrief_marx_an_bankert.docx) | Mandantenbrief Dr. Marx an Bankert (JVA) — Verfahrensstand, Verständigung |
| 17 | [`17_email_korrespondenz_marx_staatsanwaltschaft.docx`](17_email_korrespondenz_marx_staatsanwaltschaft.docx) | Übersicht E-Mail-Korrespondenz mit Inhaltsangaben (Verweise auf eml/) |
| 18 | [`18_pressemitteilung_staatsanwaltschaft_redacted.docx`](18_pressemitteilung_staatsanwaltschaft_redacted.docx) | Pressemitteilung StA Frankfurt Nr. 12/2025 (redigiert) + Kanzleivermerk |
| 19 | [`19_einlassung_entwurf_bankert.docx`](19_einlassung_entwurf_bankert.docx) | Einlassung des Angeklagten — Entwurf (intern, nicht autorisiert) |
| 20 | [`20_adhaesionsantrag_abc_3_8_mio.docx`](20_adhaesionsantrag_abc_3_8_mio.docx) | Adäsionsantrag ABC SE 3.800.000 EUR — §§ 403 ff. StPO, Gegenstrategie |
| 21 | [`21_kanzleinotiz_strategiekonferenz.docx`](21_kanzleinotiz_strategiekonferenz.docx) | Strategiekonferenz 20.05.2025 — Vollständige HV-Strategie aller Anklagepunkte |
| 22 | [`22_fristenkalender_hauptverhandlung_lg.docx`](22_fristenkalender_hauptverhandlung_lg.docx) | Fristenkalender HV-Beginn 23.06.2025 — Termine, Checkliste, Revisionshinweise |

---

## Anhänge

### DOCX

| Datei | Inhalt |
|---|---|
| [`docx/akteneinsichtsantrag_marx_147_stpo.docx`](docx/akteneinsichtsantrag_marx_147_stpo.docx) | Antrag auf Akteneinsicht § 147 StPO — vollständiger Schriftsatz Dr. Marx |
| [`docx/haftbeschwerde_304_stpo_bankert.docx`](docx/haftbeschwerde_304_stpo_bankert.docx) | Haftbeschwerde §§ 304, 117 StPO — Schriftsatz mit Anlagen |
| [`docx/schriftsatz_widerspruch_mandatsdaten_97_148_stpo.docx`](docx/schriftsatz_widerspruch_mandatsdaten_97_148_stpo.docx) | Schriftsatz Widerspruch Mandatsdaten-Sicherstellung §§ 97, 148 StPO |

### XLSX

| Datei | Inhalt |
|---|---|
| [`xlsx/schadenstabelle_betrug_tatkomplex.xlsx`](xlsx/schadenstabelle_betrug_tatkomplex.xlsx) | Schadenstabelle Betrug: 14 Tatkomplexe, Kaufpreise, Anklage vs. Verteidigung |
| [`xlsx/steuerhinterziehung_berechnung_vz2019_2022.xlsx`](xlsx/steuerhinterziehung_berechnung_vz2019_2022.xlsx) | Steuerhinterziehungsberechnung VZ 2019–2022 (KSt, GewSt, USt) + Strafrahmenanalyse |

### E-Mails (.eml)

| Datei | Inhalt |
|---|---|
| [`eml/01_marx_an_sta_akteneinsicht_25032025.eml`](eml/01_marx_an_sta_akteneinsicht_25032025.eml) | Marx an StA Wallisch: Akteneinsichtsantrag, 25.03.2025 |
| [`eml/02_sta_an_marx_akteneinsicht_01042025.eml`](eml/02_sta_an_marx_akteneinsicht_01042025.eml) | StA Wallisch an Marx: Akteneinsicht gewährt, 01.04.2025 |
| [`eml/03_marx_an_kanzleirezeption_08052025.eml`](eml/03_marx_an_kanzleirezeption_08052025.eml) | Marx an Kanzleirezeption: Medienanfragen, Termine, 08.05.2025 |
| [`eml/04_sta_an_marx_haftbeschwerde_12052025.eml`](eml/04_sta_an_marx_haftbeschwerde_12052025.eml) | StA Wallisch an Marx: Stellungnahme Haftbeschwerde, 12.05.2025 |
| [`eml/05_kraemer_an_marx_adhaesion_vergleich_20052025.eml`](eml/05_kraemer_an_marx_adhaesion_vergleich_20052025.eml) | RA Krämer an Marx: Gesprächsangebot Zivilvergleich, Adhäsionsantrag ABC SE, 20.05.2025 |

### PDFs

> Diese Testakte wurde mit KI generiert und ist ein Experiment. Benutzung auf eigene Verantwortung und eigene Gefahr.
>
> This test case file was generated with AI and is an experiment. Use at your own responsibility and risk.

| Datei | Inhalt |
|---|---|
| [`pdfs/haftbefehl_auszug_redigiert_lg_frankfurt.pdf`](pdfs/haftbefehl_auszug_redigiert_lg_frankfurt.pdf) | Haftbefehl LG Frankfurt — redigierter Auszug (Verteidigungskopie) |
| [`pdfs/pressemitteilung_sta_frankfurt_17032025_redigiert.pdf`](pdfs/pressemitteilung_sta_frankfurt_17032025_redigiert.pdf) | Pressemitteilung StA Frankfurt Nr. 12/2025 — redigierte PDF-Fassung |

### Fotos / Illustrationen

| Datei | Inhalt |
|---|---|
| [`jpg/organigramm_bankert_industrieautomation.jpg`](jpg/organigramm_bankert_industrieautomation.jpg) | Organigramm Bankert Industrieautomation GmbH, Gesellschafter, Trust, Nebenklägerin |
| [`jpg/jva_frankfurt_i_aussenansicht_illustration.jpg`](jpg/jva_frankfurt_i_aussenansicht_illustration.jpg) | JVA Frankfurt I — Illustration Außenansicht (kein Originalfoto) |
| [`jpg/geldfluss_diagramm_bankert_tatkomplexe.jpg`](jpg/geldfluss_diagramm_bankert_tatkomplexe.jpg) | Geldfluss-Diagramm: Betrug, Untreue, Bestechung, Steuerhinterziehung |

---

## Verfahrensstand

**Stichtag dieser Akte: Mai 2025**
**Mandatsannahme: 20. März 2025**

| Verfahrensstrang | Status |
|---|---|
| Untersuchungshaft | Seit 17.03.2025, JVA Frankfurt I; Haftbeschwerde abgelehnt (16.05.2025) |
| Außervollzugsetzung § 116 StPO | In Prüfung; Bankbürgschaft 500.000 EUR in Vorbereitung |
| Akteneinsicht | 14 Bände vollständig erhalten (02.04.2025), ausgewertet |
| Anklageschrift | Zugestellt 10.04.2025 |
| Verständigung § 257c StPO | Angebot StA vom 28.04.2025; Gegenvorschlag Verteidigung 22.05.2025 |
| Nebenklage ABC SE | Beantragt 05.05.2025; Zulassung durch Kammer ausstehend |
| Adäsionsantrag | 3.800.000 EUR; Gegenstrategie vorbereitet |
| Beweisantragsschriften | In Vorbereitung, Einreichung bis 09.06.2025 |
| Gegengutachten Maschinenbewertung | Beauftragung läuft (Prof. Helbig, TU Darmstadt) |
| Gegengutachten Anlage B14 | Beauftragung läuft (Schriftvergleich) |
| Hauptverhandlung | Beginn 23.06.2025, geplant 11 HV-Tage |
| Gesellschafterstreit Geweke | Angekündigt; gesellschaftsrechtliche Beratung RAin Dabel |
| Insolvenzgefahr BIA GmbH | Akut; Restrukturierungsberater eingeschaltet |

**Verteidigerin:** Dr. Friederike Marx, Fachanwältin für Strafrecht, Marx Strafverteidigung Frankfurt, Bockenheimer Landstraße 84, 60323 Frankfurt am Main
**Staatsanwaltschaft:** StA Frankfurt am Main, OStA Dr. Henning Wallisch, Az. 5400 Js 14782/24
**Gericht:** LG Frankfurt am Main, 14. Gr. WiStK, Az. 5/28 KLs 122/25
