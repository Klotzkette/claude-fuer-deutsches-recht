# Akte: NDA-Vertragsabgleich Windsysteme Norderhof AG / Eickmann Wirtschaftspartner Pte. Ltd. — Joint Venture, GeschGehG, Exportkontrolle

<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

Dieses Aktenpaket gibt es in mehreren Formaten zum Direkt-Download. Das Gesamt-PDF eignet sich zum Lesen, Ausdrucken und für schnelle Durchsichten. Das Akten-ZIP enthält sämtliche Originaldateien (DOCX-Aktenstücke mit Briefkopf, Tabellen, E-Mails, Fotos, PDFs, XLSX) im Originalordnerlayout für eigene Auswertungen. Das Einzel-PDF-ZIP liefert jede einzelne Unterlage als separate, sauber gerenderte PDF im Originalordnerlayout — praktisch, wenn nur einzelne Aktenstücke gebraucht werden.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei, 517 KB) | PDF | [`gesamt-pdf/nda-vertragsabgleich-jointventure-windsysteme-eickmann-wirtschaft_gesamt.pdf`](gesamt-pdf/nda-vertragsabgleich-jointventure-windsysteme-eickmann-wirtschaft_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-nda-vertragsabgleich-jointventure-windsysteme-eickmann-wirtschaft.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-nda-vertragsabgleich-jointventure-windsysteme-eickmann-wirtschaft.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-nda-vertragsabgleich-jointventure-windsysteme-eickmann-wirtschaft-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-nda-vertragsabgleich-jointventure-windsysteme-eickmann-wirtschaft-einzelpdfs.zip) |

Die ZIP-URLs sind stabil und zeigen immer auf die aktuelle Version. Im Akten-ZIP ist das Gesamt-PDF mit enthalten.

<!-- END gesamt-pdf-section (autogen) -->

Arbeitsakte zum Plugin `nda-abgleich`.

---

## Kurzbild

- Mandant: **Windsysteme Norderhof AG**, Norderstedt, Offshore-Windkraftkomponenten, ca. 1.100 Mitarbeiter; CEO Dr. Henning Norderhof, CTO Dr. Iva Tannenmoor, CFO Dr. Aram Eichenmuller.
- Gegenseite: **Eickmann Wirtschaftspartner Pte. Ltd.**, Singapur — Joint-Venture-Partner für den Asien-Pazifik-Markt (Offshore-Wind, Iran-Komponenten).
- Kanzlei: **Schwingenstein und Partner**, Hamburg; Bearbeiter: RA Dr. Mark Roosendaal; AZ: MR-2026-1118.
- BAFA-Vorgang: Az. 4022-26/Iran-WS-188 (Dual-Use-Prüfung, AWG/AWV, VO (EU) 833/2014, VO (EU) 267/2012).
- Kernproblem: Eickmann legt einen NDA-Entwurf vor, der acht kritische Konfliktlinien enthält; Version 7 ist das Verhandlungsergebnis nach zwei Verhandlungsrunden in Singapur.

---

## Konfliktstränge

1. **Laufzeit**: Eickmann fordert 7 Jahre, Windsysteme will 3 Jahre — Kompromiss bei 5 Jahren unter Bedingungen.
2. **Residual-Information-Klausel**: Eickmanns Entwurf erlaubt Mitarbeitern das unbewusste Weiterverwenden erlernter Informationen — für Windsysteme nicht akzeptabel.
3. **Mitarbeiter-Abwerbe-Klausel**: Vertragsstrafe von 250.000 EUR je Abwerbung, Windsysteme will Deckelung auf 75.000 EUR.
4. **Exportkontrolle / Iran**: Projekt berührt Iran-Sanktionen (VO (EU) 267/2012, AWG/AWV); BAFA-Vorabauskunft läuft unter Az. 4022-26/Iran-WS-188.
5. **Gerichtsstand**: Eickmann besteht auf SIAC-Schiedsklausel (Singapur); Windsysteme will LG Hamburg, subsidiär DIS.
6. **GeschGehG-Konformität**: Sind die vereinbarten IT-Sicherheitsmaßnahmen „angemessen" i.S.d. § 2 Nr. 1 lit. b GeschGehG?
7. **Konzernweite Restrukturierungsklausel**: Eickmann-Entwurf erlaubt Übertragung der NDA-Rechte auf Konzerntöchter ohne Zustimmung; für Windsysteme untragbar.
8. **Cybertrading-Bedenken**: IT-Security-Audit ergibt Hinweise auf Cybertrading-Netzwerk im Umfeld einer Eickmann-Beteiligung; RA Roosendaal gibt Warnhinweis.

---

## Auswertungsschwerpunkte

| Skill | Aktenstück | Demonstration |
|---|---|---|
| NDA-Vertragsanalyse | 03, 04 | Primärentwurf vs. Gegenentwurf, Strukturvergleich |
| Klauselabgleich (Redline) | 05–14 | Je Klausel: Eickmann-Fassung, Windsysteme-Fassung, Differenzanalyse, Risikobewertung |
| GeschGehG §§ 2–3 | 16 | Angemessenheit der Geheimhaltungsmaßnahmen, Audit-Checkliste |
| Exportkontrolle AWG/AWV | 11, 18 | Dual-Use, Iran-Sanktionen VO (EU) 267/2012, BAFA-Vorabauskunft |
| Gerichtsstand-Streit | 15 | SIAC Rules 2016, LG Hamburg, DIS, Rechtswahlklausel |
| Vertragsstrafe / AGB-Kontrolle | 08 | §§ 305 ff. BGB, § 307 BGB, Angemessenheitsprüfung |
| Residual-Information | 12 | Klausel-Risiko, Know-how-Schutz, GeschGehG |
| Mitarbeiterklausel | 13 | Abwerbeverbote, Vertragsstrafe, Reichweite |
| Konzernrestrukturierung | 14 | Übertragungsklausel, Change-of-Control, Einwilligungsvorbehalt |
| Verhandlungsprotokoll | 19, 20 | Zwei Verhandlungstage, Positionen, Bewegungen |
| Finale NDA-Fassung | 21 | Konsolidierter Text v7, zweisprachig DE/EN |
| Abschlussvermerk | 22 | Beratungsergebnis, offene Punkte, Empfehlungen |

---

## Aktenstücke

| Nr. | Datei | Inhalt |
|---|---|---|
| 01 | [`01-mandatsuebernahme-windsysteme.docx`](01-mandatsuebernahme-windsysteme.docx) | Mandatsaufnahme, Vollmacht, Interessenkonfliktprüfung |
| 02 | [`02-projektbeschreibung-jointventure.docx`](02-projektbeschreibung-jointventure.docx) | JV-Struktur, Beteiligungsverhältnisse, Hintergrund |
| 03 | [`03-nda-entwurf-eickmann-version-1.docx`](03-nda-entwurf-eickmann-version-1.docx) | Eickmanns NDA-Ersterstellung v1 (vollständig) |
| 04 | [`04-nda-gegenentwurf-windsysteme.docx`](04-nda-gegenentwurf-windsysteme.docx) | Windsystemes Gegenentwurf mit Markups |
| 05 | [`05-klauselabgleich-vertraulichkeit.docx`](05-klauselabgleich-vertraulichkeit.docx) | Klauselvergleich: Definition vertrauliche Information |
| 06 | [`06-klauselabgleich-verwendungszweck.docx`](06-klauselabgleich-verwendungszweck.docx) | Klauselvergleich: Verwendungszweck und Zweckbindung |
| 07 | [`07-klauselabgleich-rueckgabepflicht.docx`](07-klauselabgleich-rueckgabepflicht.docx) | Klauselvergleich: Rückgabe/Vernichtung, Datenträger |
| 08 | [`08-klauselabgleich-vertragsstrafe.docx`](08-klauselabgleich-vertragsstrafe.docx) | Klauselvergleich: Vertragsstrafe, AGB-Kontrolle § 307 BGB |
| 09 | [`09-klauselabgleich-laufzeit.docx`](09-klauselabgleich-laufzeit.docx) | Klauselvergleich: Laufzeit 7 vs. 3 Jahre, Kompromiss |
| 10 | [`10-klauselabgleich-gerichtsstand-recht.docx`](10-klauselabgleich-gerichtsstand-recht.docx) | Klauselvergleich: Rechtswahlklausel, Gerichtsstand |
| 11 | [`11-klauselabgleich-exportkontrolle-iran.docx`](11-klauselabgleich-exportkontrolle-iran.docx) | Exportkontroll-Klausel, AWG/AWV, Iran-Sanktionen |
| 12 | [`12-klauselabgleich-residual-information.docx`](12-klauselabgleich-residual-information.docx) | Residual-Information-Klausel, GeschGehG-Risiko |
| 13 | [`13-klauselabgleich-mitarbeiterklausel.docx`](13-klauselabgleich-mitarbeiterklausel.docx) | Abwerbeverbote, Vertragsstrafe 250.000 EUR, Deckelung |
| 14 | [`14-klauselabgleich-restrukturierung-konzern.docx`](14-klauselabgleich-restrukturierung-konzern.docx) | Konzernübertragungsklausel, Change-of-Control |
| 15 | [`15-streitstand-gerichtsstand-singapur.docx`](15-streitstand-gerichtsstand-singapur.docx) | SIAC Rules 2016, LG Hamburg, DIS, Vergleich |
| 16 | [`16-geschgehg-pruefung-angemessene-massnahmen.docx`](16-geschgehg-pruefung-angemessene-massnahmen.docx) | GeschGehG §§ 2–3, Audit-Checkliste, Empfehlungen |
| 17 | [`17-eckert-marker-tabelle-fallstricke.docx`](17-eckert-marker-tabelle-fallstricke.docx) | Marker-Tabelle: kritische Klauselrisiken im Überblick |
| 18 | [`18-cybertrading-warnliste-fuer-mandant.docx`](18-cybertrading-warnliste-fuer-mandant.docx) | IT-Security-Hinweise, Cybertrading-Bedenken |
| 19 | [`19-protokoll-verhandlung-tag-1.docx`](19-protokoll-verhandlung-tag-1.docx) | Verhandlungsprotokoll Tag 1, Singapur |
| 20 | [`20-protokoll-verhandlung-tag-2.docx`](20-protokoll-verhandlung-tag-2.docx) | Verhandlungsprotokoll Tag 2, Singapur |
| 21 | [`21-finale-version-nda-v7.docx`](21-finale-version-nda-v7.docx) | Konsolidierte NDA-Fassung v7 (Textfassung MD) |
| 22 | [`22-abschlussvermerk-anwalt.docx`](22-abschlussvermerk-anwalt.docx) | Anwaltlicher Abschlussvermerk, Empfehlungen |

---

## Anhänge

### DOCX

| Datei | Inhalt |
|---|---|
| [`docx/nda-finale-version-v7-windsysteme-eickmann.docx`](docx/nda-finale-version-v7-windsysteme-eickmann.docx) | NDA Endversion v7, deutsch/englisch zweispaltig |
| [`docx/klauselabgleich-redline-version-5.docx`](docx/klauselabgleich-redline-version-5.docx) | Redline-Fassung v5 mit Markierungen aller Änderungen |
| [`docx/vorstandsvorlage-windsysteme.docx`](docx/vorstandsvorlage-windsysteme.docx) | Vorstandsvorlage: Handlungsempfehlungen für CEO/CFO/CTO |

### XLSX

| Datei | Inhalt |
|---|---|
| [`xlsx/klausel-vergleichs-matrix.xlsx`](xlsx/klausel-vergleichs-matrix.xlsx) | Klauselvergleich: Eickmann vs. Windsysteme, Risiko, Empfehlung |
| [`xlsx/exportkontrolle-pruefung.xlsx`](xlsx/exportkontrolle-pruefung.xlsx) | Exportkontroll-Prüfmatrix: Dual-Use, Iran, BAFA-Lizenzpflicht |

### E-Mails (.eml)

| Datei | Inhalt |
|---|---|
| [`emails/email-eickmann-an-windsysteme-nda-v1.eml`](emails/email-eickmann-an-windsysteme-nda-v1.eml) | Eickmann übermittelt NDA-Erststentwurf v1 |
| [`emails/email-windsysteme-rueckspielung-v2.eml`](emails/email-windsysteme-rueckspielung-v2.eml) | Windsysteme sendet Gegenentwurf v2 zurück |
| [`emails/email-anwalt-bedenken-residual-clause.eml`](emails/email-anwalt-bedenken-residual-clause.eml) | RA Roosendaal: rechtliche Bedenken Residual-Klausel |
| [`emails/email-bafa-vorabauskunft-iran-projekt.eml`](emails/email-bafa-vorabauskunft-iran-projekt.eml) | BAFA-Vorabauskunft Az. 4022-26/Iran-WS-188 |
| [`emails/email-verhandlungseinladung-singapur.eml`](emails/email-verhandlungseinladung-singapur.eml) | Einladung Verhandlungstage Singapur, SIAC-Tagungsraum |

### PDFs

| Datei | Inhalt |
|---|---|
| [`pdfs/nda-finale-version-v7-final.pdf`](pdfs/nda-finale-version-v7-final.pdf) | NDA v7 als druckfertiges PDF |
| [`pdfs/klauselabgleich-zusammenfassung.pdf`](pdfs/klauselabgleich-zusammenfassung.pdf) | Zusammenfassung aller Klauselvergleiche für Vorstand |

### Fotos

| Datei | Inhalt |
|---|---|
| [`jpg/verhandlungsraum-singapur-marina-bay.jpg`](jpg/verhandlungsraum-singapur-marina-bay.jpg) | Verhandlungsraum Marina Bay (Referenzfoto) |
| [`jpg/kommentierter-nda-screenshot.jpg`](jpg/kommentierter-nda-screenshot.jpg) | Annotierter NDA-Screenshot mit Markierungen |
| [`jpg/prozess-flowchart-nda-eskalation.jpg`](jpg/prozess-flowchart-nda-eskalation.jpg) | Prozess-Flowchart NDA-Eskalationspfade |

---

## Verfahrensstand

**Stichtag dieser Akte: Juni 2026**
**Mandatsannahme: 15. Januar 2026**

| Verfahrensstrang | Status |
|---|---|
| NDA-Verhandlung | Abgeschlossen; NDA v7 paraphiert |
| BAFA-Vorabauskunft | Az. 4022-26/Iran-WS-188; Antwort ausstehend |
| Gerichtsstands-Kompromiss | DIS Hamburg vereinbart (Eickmann hat SIAC fallen gelassen) |
| GeschGehG-Audit | Maßnahmen-Paket verabschiedet; Umsetzung Q3 2026 |
| Residual-Klausel | Gestrichen auf Windsystemes Betreiben |
| Konzernübertragungsklausel | Zustimmungsvorbehalt Windsysteme durchgesetzt |
| Mitarbeiter-Abwerbe-Vertragsstrafe | Kompromiss: 120.000 EUR je Vorfall, Jahresdeckel 480.000 EUR |
| Cybertrading-Prüfung | IT-Security-Bericht angefordert; Ergebnis offen |

Bearbeiter: **RA Dr. Mark Roosendaal**, Schwingenstein und Partner, Hamburg
