# Akte Umweltrecht: Industrieanlage — Genehmigung, Emissionshandel, Altlast und Transaktion

<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

[Testakten-Übersicht](../README.md) · [Repository-Start](../../README.md) · [Plugin-Katalog](../../README.md#was-ist-drin) · [Download-Index](../../ASSET_INDEX.md)

Dieses Aktenpaket gibt es in drei Formaten. Das Gesamt-PDF eignet sich zum Lesen und Ausdrucken. Das Akten-ZIP enthält die nativen Originaldateien wie DOCX, Tabellen, E-Mails, Fotos und PDFs. Es enthält kein Markdown; sämtliche Dateien liegen ohne Unterordner unmittelbar auf der ZIP-Wurzelebene. Das Einzel-PDF-ZIP liefert jede Unterlage als separate, sauber gerenderte PDF unmittelbar auf der ZIP-Wurzelebene.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei) | PDF | [`gesamt-pdf/umweltrecht-industrieanlage-genehmigung_gesamt.pdf`](gesamt-pdf/umweltrecht-industrieanlage-genehmigung_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-umweltrecht-industrieanlage-genehmigung.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-umweltrecht-industrieanlage-genehmigung.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-umweltrecht-industrieanlage-genehmigung-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-umweltrecht-industrieanlage-genehmigung-einzelpdfs.zip) |

Die ZIP-Links laden den zuletzt veröffentlichten Release. Das Gesamt-PDF ist auch im Akten-ZIP enthalten; für eine einheitliche Arbeitsfassung genügt deshalb dieses Archiv. Der hier verlinkte Repository-Stand kann zwischen Releases bereits neuer sein.

English: The original-format ZIP contains the working files directly at archive root, without subfolders or Markdown. Choose the combined PDF for reading; it is also included in that ZIP. Choose the individual-PDF ZIP to review each document separately. These are practice documents, not an installable plugin. ZIP links refer to the latest published release.

<!-- END gesamt-pdf-section (autogen) -->

## ⬇️ Direkt-Download

| Akte | Direkt-Download |
| --- | --- |
| `testakte-umweltrecht-industrieanlage-genehmigung` (Akte) | [testakte-umweltrecht-industrieanlage-genehmigung.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-umweltrecht-industrieanlage-genehmigung.zip) |

Diese Akte wird separat als ZIP-Datei aus dem GitHub-Release bereitgestellt. Das ZIP enthält die Originalformate (PDF, DOCX, XLSX, CSV, JPEG) für die Bearbeitung.

Arbeitsakte über einen komplexen umweltrechtlichen Beratungsfall rund um die geplante Erweiterung einer Spezialchemie-Anlage in Nordrhein-Westfalen. Die Akte deckt das gesamte Spektrum des modernen betrieblichen Umweltrechts ab: Immissionsschutz, Emissionshandel, Wasserrecht, Bodenschutz, Naturschutz, Ordnungswidrigkeiten und umweltrechtliche Due Diligence bei einem Unternehmensverkauf.

---

## Sachverhalt

Die **Rheinland Spezialchemie GmbH** (RS GmbH) betreibt seit 2007 das **Werk Nord** in Wesseling (Nordrhein-Westfalen), einem Feinchemikalien-Standort im Industriepark Wesseling-Nord. Die Anlage, genehmigt nach dem Bundes-Immissionsschutzgesetz (BImSchG) für die Herstellung von Chlorbenzaldehyd-Derivaten, umfasst vier Produktionslinien sowie ein eigenes Zentralkesselhaus, das die Anlage dem Europäischen Emissionshandelssystem (EU-ETS / TEHG) unterwirft.

Im Frühjahr 2026 plant die RS GmbH die Erweiterung der **Linie 4** um eine neue Inline-Trocknungsstufe (Schaufeltrockner Hosokawa Alpine CT-600), die bislang ausgelagerte und emissionsintensive Vortrocknung von Zwischenprodukten in den geschlossenen Anlagenbetrieb zu verlagern. Für dieses Vorhaben stellt sich zunächst die Frage, ob eine bloße **Änderungsanzeige nach § 15 BImSchG** ausreicht oder eine **Änderungsgenehmigung nach § 16 BImSchG** erforderlich ist. Die Anzeige wurde am 14. März 2026 mit dem Hilfsantrag auf Behandlung als Genehmigungsantrag eingereicht. Parallel dazu berührt die Erweiterung die Emissionshandelsposition der Anlage (TEHG), die Abwassersituation und eine bisher nicht untersuchte Altlastverdachtsfläche (ehemalige Galvanikanlage in der Südhalle S-02).

Die Situation wird durch zwei weitere Konfliktlinien verschärft: Zum einen hat die Bezirksregierung Köln Ende März 2026 ein **Bußgeldverfahren** eingeleitet, weil die RS GmbH eine dreitägige Grenzwertüberschreitung am Messpunkt Mahlanlage (Schornstein H-02) — verursacht durch einen Schlauchfilter-Schaden — erst zwölf Tage nach Bekanntwerden gemeldet hatte. Die Verteidigung setzt auf den strittigen Beginn der 24-Stunden-Meldefrist und auf den Messgeräteausfall als Erkenntnishindernis. Zum anderen hat ein lokaler **Umweltverband** (Rheinischer Umweltschutzverein e.V.) Zugang zu den Genehmigungsunterlagen, Emissionsberichten und Altlastendokumenten beantragt — ein UIG-Verfahren, das zeitlich ungünstig mit dem laufenden Erweiterungsverfahren zusammenfällt.

Schließlich prüft ein Investor die Übernahme von Gesellschaftsanteilen an der RS GmbH. Im Rahmen der **Umwelt-Due-Diligence** werden alle bekannten und latenten Risiken aus Genehmigungsbestand, Altlasten, Emissionshandel, Abfallnachweisen und wasserrechtlichen Erlaubnissen systematisch durchleuchtet. Das Red-Flag-Memo identifiziert als Hauptrisiko die ungeklärte Altlast in Südhalle S-02 sowie die fehlende Einstufung und den fehlenden eANV-Nachweis für ca. 950 t/a Produktionsschlamm.

---

## Eckdaten

| Merkmal | Wert |
|---|---|
| Betreiberin | Rheinland Spezialchemie GmbH |
| Standort | Industriestraße 47, 50389 Wesseling (NRW) — Aktenadresse |
| Anlagentyp | Feinchemikalienproduktion (Chlorbenzaldehyd-Derivate), Batch-Verfahren |
| BImSchG-Kategorie | Nr. 4.1.1 Anhang 1 4. BImSchV, IED-Anlage |
| Zuständige Behörde | Bezirksregierung Köln, Abt. 6 (Immissionsschutz) |
| Genehmigungszeichen (Bestand) | BR-K/6-52.4/2019-014 |
| Genehmigungszeichen (Änderungsanzeige) | BR-K/6-52.4/2026-031 |
| TEHG-Anlage-ID | DE-RS-GmbH-0047-2021 |
| OWi-Verfahren | BR-K/6-OWi-2026-0087 |
| Geschäftsführung RS GmbH | Dr. Markus Haverkamp (technisch), Sandra Vielhaber (kaufmännisch)  |
| Beratende Kanzlei | Rosengart & Partner, Köln |
| UIG-Antragstellerin | Rheinischer Umweltschutzverein e.V. |

### Zeitleiste

| Datum | Ereignis |
|---|---|
| 12.11.2007 | Errichtungs- und Betriebsgenehmigung Werk Nord |
| 03.09.2019 | Änderungsgenehmigung Linie 1–3 |
| 04.02.2021 | Erneuerung Einleitungserlaubnis WHG (bis 31.12.2027) |
| 15.02.2024 | Genehmigung Monitoring-Plan Revision 3 (TEHG) |
| 04.–06.03.2026 | Grenzwertüberschreitung Staub Schornstein H-02 (Schlauchfilterausfall) |
| 07.03.2026 | Filterschaden behoben |
| 14.03.2026 | Änderungsanzeige § 15 BImSchG Linie-4-Erweiterung eingereicht |
| 18.03.2026 | Verspätete Meldung KEÜ-Überschreitung an Bezirksregierung |
| 20.03.2026 | Mitteilung Anlagenänderung und MP-Revisionsantrag an DEHSt |
| 22.03.2026 | Artenschutzrechtliche Vorprüfung (Stufe I) fertiggestellt |
| 25.03.2026 | Altlastanzeige Südhalle S-02 nach § 9 BBodSchG |
| 02.04.2026 | Anhörungsschreiben OWi-Verfahren von Bezirksregierung |
| 03.04.2026 | UIG-Antrag Rheinischer Umweltschutzverein |
| 08.04.2026 | Umwelt-DD-Fragenliste und Red-Flag-Memo erstellt |
| 22.04.2026 | Verteidigungsschreiben OWi eingereicht |
| 30.06.2026 | Ziel: Einreichung Monitoring-Plan Revision 4 bei DEHSt |
| 01.08.2026 | Geplante Inbetriebnahme Trocknungsstufe |
| 30.09.2026 | Geplanter Abschluss Altlastenuntersuchung S-02 |

---

## Dateien in dieser Akte

| Ordner / Datei | Inhalt |
|---|---|
| [01_intake/email_anlagenbetreiber.eml](01_intake/email_anlagenbetreiber.eml) | Vollständiges E-Mail-Briefing zu Linie 4, Beschwerden, UIG-Antrag und Anhörung |
| [01_intake/telefonvermerk_bergerhausen_fundament.docx](01_intake/telefonvermerk_bergerhausen_fundament.docx) | Telefonvermerk der Kanzlei: Fundamenttermin, verschobene Trocknerlieferung, behördliche Nachforderung, neue Geruchsbeschwerde |
| [02_bimschg/aenderungsanzeige_roh.docx](02_bimschg/aenderungsanzeige_roh.docx) | Vollständige Änderungsanzeige nach § 15 BImSchG mit Briefkopf, Anlagenbeschreibung, Emissionsprognose, Abgrenzung § 16 BImSchG, Nachbarschaftsstellungnahme |
| [02_bimschg/aenderungsanzeige_roh.docx](02_bimschg/aenderungsanzeige_roh.docx) | DOCX-Fassung der Änderungsanzeige (Word-Dokument) |
| [03_tehg/zuteilung_dehst_notiz.docx](03_tehg/zuteilung_dehst_notiz.docx) | Interner Aktenvermerk zur TEHG-Kapazitätsänderungsprüfung + förmliches DEHSt-Schreiben zur Monitoring-Plan-Revision 4 |
| [03_tehg/zuteilung_dehst_notiz.docx](03_tehg/zuteilung_dehst_notiz.docx) | DOCX-Fassung des DEHSt-Schreibens |
| [04_abfall/stoffstromliste.csv](04_abfall/stoffstromliste.csv) | Abfallstrom-Bilanz aller Fraktionen (EAK-Schlüssel, Menge, Einstufung, Entsorger, eANV-Status) |
| [05_wasser_boden/einleitung_altlast.docx](05_wasser_boden/einleitung_altlast.docx) | Fachlicher Vermerk zur WHG-Einleitungserlaubnis (Kapazitätsänderung) und Anzeige des Altlastenverdachts Südhalle S-02 nach § 9 BBodSchG |
| [05_wasser_boden/rammkernsondierung_s02_messwerte.csv](05_wasser_boden/rammkernsondierung_s02_messwerte.csv) | Labor-Messwerte der orientierenden Untersuchung Südhalle S-02 (Boden- und Grundwasserproben, ein unplausibler Wert in Wiederholung) |
| [06_naturschutz/artenschutz_hinweis.docx](06_naturschutz/artenschutz_hinweis.docx) | Artenschutzrechtliche Vorprüfung (Stufe I) nach § 44 BNatSchG: Fledermäuse, Vogelarten, Zauneidechse, FFH-Bezug |
| [07_uig_ifg/uig_antrag_umweltverband.txt](07_uig_ifg/uig_antrag_umweltverband.txt) | UIG-Antrag des Rheinischen Umweltschutzvereins auf Zugang zu Genehmigungsunterlagen, Emissionsdaten, Geruchsgutachten und Altlastdokumenten |
| [07_uig_ifg/zwischennachricht_bezirksregierung_uig.docx](07_uig_ifg/zwischennachricht_bezirksregierung_uig.docx) | Zwischennachricht der Bezirksregierung zum UIG-Antrag: Fristverlängerung, Drittbeteiligung der RS GmbH, gesonderte Prüfung des OWi-Vorgangs |
| [08_bussgeld/anhoerung_verspätete_meldung.docx](08_bussgeld/anhoerung_verspaetete_meldung.docx) | Anhörungsschreiben der Bezirksregierung (OWi § 62 BImSchG) + Verteidigungsschreiben der Kanzlei Rosengart & Partner |
| [08_bussgeld/anhoerung_verspätete_meldung.docx](08_bussgeld/anhoerung_verspaetete_meldung.docx) | DOCX-Fassung Anhörung und Verteidigungsentwurf |
| [09_transaktion/umwelt_dd_roh.docx](09_transaktion/umwelt_dd_roh.docx) | Umwelt-Due-Diligence-Fragenliste (7 Abschnitte, inkl. Antworten) + Red-Flag-Memo mit Risikomatrix für Anteilskäufer |

---

## Prüffokus

Diese Akte vermittelt typische Querschnittsthemen des **betrieblichen Umweltrechts** in Deutschland, die in der Beratungspraxis häufig gleichzeitig auftreten:

- **BImSchG-Anzeige vs. Genehmigung (§ 15 vs. § 16 BImSchG):** Wie wird die Genehmigungspflicht abgegrenzt? Welche Unterlagen braucht eine Änderungsanzeige? Wann ist ein Hilfsantrag sinnvoll?
- **TEHG-Emissionshandel:** Wann löst eine Kapazitätsänderung eine wesentliche Änderungsmeldepflicht aus? Wie funktioniert die Monitoring-Plan-Revision?
- **WHG und Einleitungserlaubnisse:** Was sind die Grenzwerte, wann muss eine Erlaubnis angepasst werden, und was gilt bei befristeten Erlaubnissen?
- **BBodSchG / Altlasten:** Wann muss eine Altlastverdachtsfläche angezeigt werden? Wer trägt welche Sanierungsverantwortung?
- **Artenschutzrecht (§ 44 BNatSchG, FFH):** Was ist eine artenschutzrechtliche Vorprüfung? Welche Kartierungspflichten bestehen? Wie wirkt sich Fledermausvorkommen auf Bauzeiten aus?
- **Ordnungswidrigkeitenrecht (OWiG, § 62 BImSchG):** Wie ist der Ablauf einer OWi-Anhörung? Welche Verteidigungsstrategien gibt es bei Meldepflichtverstößen?
- **Umweltinformationsrecht (UIG):** Welche Informationen sind zugänglich? Welche Ausschlüsse gibt es für Betriebs- und Geschäftsgeheimnisse?
- **Umwelt-Due-Diligence bei M&A:** Was sind die typischen Themen? Wie wird ein Red-Flag-Memo strukturiert? Wie schlägt sich Altlast-Unsicherheit in Kaufpreismechanismen nieder?

---

## Aktenhinweis

Personenbezogene Angaben, Adressen und Kontaktdaten sind anonymisiert; die rechtliche Arbeit richtet sich nach den in der Akte dokumentierten Unterlagen.

Erstellt von: Klotzkette / 39582916+Klotzkette@users.noreply.github.com
