# Testakte: Insolvenzanfechtung Trave Clean — Beitragszahlung an die Krankenkasse nach Fremdantrag (Lübeck)


<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

Dieses Aktenpaket gibt es in mehreren Formaten zum Direkt-Download. Das Gesamt-PDF eignet sich zum Lesen, Ausdrucken und für schnelle Durchsichten. Das Akten-ZIP enthält sämtliche Originaldateien (DOCX-Aktenstücke mit Briefkopf, Tabellen, E-Mails, Fotos, PDFs, XLSX) im Originalordnerlayout für eigene Auswertungen. Das Einzel-PDF-ZIP liefert jede einzelne Unterlage als separate, sauber gerenderte PDF im Originalordnerlayout — praktisch, wenn nur einzelne Aktenstücke gebraucht werden.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei, 55 KB) | PDF | [`gesamt-pdf/insolvenzanfechtung-kongruente-deckung-krankenkasse-nach-antrag-luebeck_gesamt.pdf`](gesamt-pdf/insolvenzanfechtung-kongruente-deckung-krankenkasse-nach-antrag-luebeck_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-insolvenzanfechtung-kongruente-deckung-krankenkasse-nach-antrag-luebeck.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-insolvenzanfechtung-kongruente-deckung-krankenkasse-nach-antrag-luebeck.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-insolvenzanfechtung-kongruente-deckung-krankenkasse-nach-antrag-luebeck-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-insolvenzanfechtung-kongruente-deckung-krankenkasse-nach-antrag-luebeck-einzelpdfs.zip) |

Die ZIP-URLs sind stabil und zeigen immer auf die aktuelle Version. Im Akten-ZIP ist das Gesamt-PDF mit enthalten.

<!-- END gesamt-pdf-section (autogen) -->

Arbeitsakte des Insolvenzverwalters über das Vermögen der Trave Clean Gebäudeservice GmbH (Gebäudereinigung, Lübeck), Amtsgericht Lübeck, 53c IN 21/26. Eine Wettbewerberin stellt am 09.02.2026 einen Fremdantrag; siebzehn Tage später — vor Anordnung von Sicherungsmaßnahmen und vor jeder Veröffentlichung im Bekanntmachungsportal — zahlt die Schuldnerin ihre rückständigen Gesamtsozialversicherungsbeiträge von 54.320 EUR an die BKK Ostseeküste. Der Verwalter ficht die kongruente Deckung nach § 130 Abs. 1 Satz 1 Nr. 2 InsO an.

Der Streit dreht sich allein um die Kenntnis der Kasse: Ein Außendienstmitarbeiter hatte neun Tage vor der Zahlung einen Telefonvermerk über den „laufenden Insolvenzantrag eines Wettbewerbers" gefertigt und an den Beitragseinzug gemailt; die Kasse beruft sich darauf, dass die zuständige Sachbearbeitung davon keine Kenntnis genommen habe, und auf den entwertenden Kontext („reine Schikane"). Zur Insolvenzreife enthält die Akte ein formal poliertes Kurzgutachten eines Sanierungsberaters vom Mai 2025, das schon zum 31.03.2025 — annähernd ein Jahr vor dem Antrag — eingetretene Zahlungsunfähigkeit und eine negative Fortbestehensprognose feststellt, während das Verwalterschreiben die nach außen manifeste Krise erst ab Januar 2026 datiert. Die Akte nimmt die rechtliche Würdigung nicht vorweg.

## Beteiligte

| Rolle | Person / Stelle |
| --- | --- |
| Schuldnerin | Trave Clean Gebäudeservice GmbH, Fackenburger Allee 112, 23554 Lübeck, HRB 19224 HL (AG Lübeck) |
| Geschäftsführer der Schuldnerin | Deniz Albayrak |
| Fremdantragstellerin | Holstein Gebäudedienste GmbH, Bad Schwartau (Wettbewerberin), vertreten durch RAin Imke Sassenberg |
| Anfechtungsgegnerin | BKK Ostseeküste, Körperschaft des öffentlichen Rechts, Kiel |
| Außendienstmitarbeiter der Kasse (bis 30.04.2026) | Malte Kröger, Firmenkundenberater |
| Teamleiterin Beitragseinzug der Kasse | Sabine Wohlert |
| Justiziarin der Kasse | Dr. Annegret Wehrmann |
| Sanierungsberater (2025) | Dr. Rehberg & Cie. Restrukturierung GmbH, Hamburg |
| Insolvenzverwalter | Rechtsanwalt Dr. Hauke Petersen, Lübeck |
| Insolvenzgericht | Amtsgericht Lübeck, 53c IN 21/26 |

## Aktenstruktur

```text
insolvenzanfechtung-kongruente-deckung-krankenkasse-nach-antrag-luebeck/
├── 01_verfahrensvermerk_verwalter.docx                     Einstiegsvermerk des Verwalters: Verfahrensgang, Anfechtungssachverhalt, Beweislage
├── 02_fremdantrag_holstein_2026-02-09.docx                 Fremdantrag der Wettbewerberin mit tituliertem Anspruch und fruchtloser Vollstreckung
├── 03_eroeffnungsbeschluss_ag_luebeck_2026-05-01.docx      Eröffnungsbeschluss des AG Lübeck mit Verwalterbestellung
├── 04_mahnung_bkk_2025-12-15.docx                          Mahnung der Einzugsstelle über den Novemberbeitrag nebst Säumniszuschlag
├── 05_vollstreckungsankuendigung_bkk_2026-01-28.docx       Vollstreckungsankündigung über 53.787,50 EUR mit Frist zum 18.02.2026
├── 06_beitragskonto_auszug_bkk.csv                         Beitragskonto Juni 2025 bis Februar 2026: verspätete Zahlungen, Rückstand, Zahlungseingang 54.320 EUR
├── 07_kontoauszug_schuldnerin_februar_2026.csv             Geschäftskonto Februar 2026: Teillöhne, Lastschriftrückgaben, Zahlung an die Kasse aus letzten Mitteln
├── 08_telefonvermerk_aussendienst_2026-02-17.docx          Telefonvermerk Kröger: Zahlungszusage und „laufender Insolvenzantrag eines Wettbewerbers"
├── 09_eidesstattliche_versicherung_kroeger_2026-06-03.docx Eidesstattliche Versicherung des ausgeschiedenen Außendienstlers zur Fertigung und Weiterleitung des Vermerks
├── 10_kurzgutachten_sanierungsberater_2025-05-12.docx      Kurzgutachten: Zahlungsunfähigkeit zum 31.03.2025, negative Fortbestehensprognose, Antragspflicht
├── 11_liquiditaetstabelle_anlage_kurzgutachten.csv         Liquiditätsstatus und Drei-Wochen-Betrachtung als Anlage zum Kurzgutachten
├── 12_recherchevermerk_bekanntmachungsportal_2026-06-04.docx Vermerk: keine Veröffentlichung vor dem 10.03.2026, Zustellung der Anhörungsverfügung am 13.02.2026
├── 13_anfechtungsschreiben_verwalter_2026-06-08.docx       Anfechtung nach § 130 Abs. 1 Satz 1 Nr. 2 InsO mit Wissenszurechnungs-Argumentation
├── 14_erwiderung_bkk_2026-06-24.docx                       Erwiderung der Kasse: keine positive Kenntnis, entwerteter Hinweis, Verlauf des Beitragskontos
└── eml/
    ├── 2026-02-18_kroeger_weiterleitung_vermerk.eml        Weiterleitung des Telefonvermerks an die Teamleiterin Beitragseinzug mit Anfechtungswarnung
    └── 2026-02-20_albayrak_zahlungsankuendigung.eml        Zahlungsankündigung des Geschäftsführers an das Funktionspostfach der Kasse
```
